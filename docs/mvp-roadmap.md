...
**The inference's load-bearing dependency:** it holds only where the compiler is proven. This roadmap's original defect list (#107, #119, #121 and the #116–#121 sentinels) records its historical planning context. For current authoring, consult the unified [authoring kit](authoring-kit.md), its explicit implementation limits and verification requirements; the former kit stage/use-case filtering no longer exists. CRE prediction and native emitted-artifact execution remain distinct evidence.
