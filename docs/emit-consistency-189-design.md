...
| B3 | `defined as exists ("X")` dispatches on `X`'s LOWERED emitted result (`emitsScalarValue`, a `totalScalarBoolean` sibling), NOT declared shape: a Scalar-VALUE operand (the B2 merge / a `most recent` value) → `(<X> is not null)` (`null-presence` discharge, total by construction); a `coded from`/RecordSet operand → `exists` (unchanged — the `Overweight Diagnoses` trap); Scalar-boolean/Record → error; cross-lib stays `exists` (scalar-value cross-lib = F). The coded both-rep interface (`code is` + `defined as exists`) stays thrown until F (asTruths removal). INERT (no corpus operand is scalar-value). disc 500. | 2026-08-23 |
