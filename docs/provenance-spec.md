...
- **`decisionReachability`**: `Map<NodeKey → {reachedBy: Set<NodeKey>, edges:[{fromDecision, fromNodeId, relation: StructuralRelation, via?}]}>` — STATIC (a node is decision-reached iff structurally referenced anywhere in a decision subtree — `when`/`otherwise`/`action`/guard, transitively through `defined as` composition, `definition is` narrative refs, `use decision`, and a reached activity's terminology), independent of any CEL run; roots = the `coversTarget`'s decisions. `StructuralRelation` (when-condition|guard|recommend|use-decision|composition-operand|definition-narrative|activity-dependency) is distinct from the artifact's authoring-intent `relation`. (Distinct from the CRE runtime trace, which only walks *reached-for-given-facts* paths.)
- **nodeKind** (pure fn): Concept+`defined as`→composition; plain/`code is`/`coded from`/`definition is`→leaf; policy-owned Activity→leaf; Terminology→`terminology` / Parameter→`parameter` (distinct tags — both over-reach-excluded; NOT `leaf`, since a `leaf` is an over-reach *candidate*); Decision sub-node (and a whole-Decision ref)→decision-node; resolved-via-shared-lib→shared-reference (overrides — but the indexer's node inventory retains the intrinsic `declKind` alongside, so the override doesn't erase the semantic kind). decisionReachability also walks `definition is` narrative concept-refs (a concept used only there is still structurally reached).
