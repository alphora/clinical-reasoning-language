...
**Operator decision (2026-08-16): they go in `qa`.** The `qa` KELP entity's folder is **`tests`**
(`kelp.project.json`), which covers both `tests/data/` and `tests/results/`. Within it they land in the
RESULTS tree, keyed by the same compartment id as the case's data:

```
tests/results/fhir/patient/<compartmentId>/questionnaire/<id>.json
tests/results/fhir/patient/<compartmentId>/questionnaireresponse/<id>.json
```

⚠ **Not `tests/data/fhir/patient/<compartmentId>/`.** `tests/data/` is what CEL emits — the facts a case
STATES — and `emit_cel` wipes and repopulates it wholesale. A second writer in that tree makes the wipe
destructive, forces an ownership marker on every file, and lets a repo bundler feed the `$apply` producer its
own previous output (`QuestionnaireResponse` is itself a legal CEL emit target). A separate tree removes all
three failure modes instead of guarding them. The `fhir/` and `patient/` segments are kept deliberately so the
results tree is itself loadable by the engine — see `results/useCases.ts`.
...
- `qa` is already a KELP entity (folder `tests`), so no new entity is needed — which HCSC KE
  flagged would otherwise be required to get them tracked. `tests/results/` is inside it.
...
Accepted cost: two generated trees under one `qa` entity, distinguished by producer rather than by entity.
