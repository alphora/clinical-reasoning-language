...
The CRE run/scenario result contract currently carries exactly one activity or pause expectation per case. CEL permits multiple non-pause result assertions (including results on several measures or several activities from an `all:` decision), but CRE reports `cre-multiple-result-assertions` instead of executing only one of them. This is a tooling contract limit, not a new CEL validation restriction. Multiple Boolean expectations also remain unsupported individually. CRE refuses the whole case before choosing any expectation, including when multiple assertions name the same Decision; the current result contract cannot represent their individual verdicts. The error has no produced activities or trace, and render_scenario reports a diagnosed error with no selected Decision/tree. If single-activity CRE cases are needed, each must retain the required facts; emitting split cases creates separate patient datasets because resource identities include the case name. No split is required for CEL data emission. Full regression and FINAL MV correspondence checks fail for these unsupported CRE cases; an MV suite using them needs supported cases before those checks can pass. Cases without an expectation and single Boolean expectations retain their explicit unsupported-execution diagnostics. Provenance references still describe authored assertions, not proof that CRE executed them; execution-path clustering defers cases with a run error.
...
`run_decision` returns raw `schemaVersion: 1`; `render_scenario` uses its separate `schemaVersion: 7`. Scenario schema 7 keeps all local branch/action nodes in entered Decision frames, but represents an unentered resolved delegation as `expanded: false, deferred: true`, with `targetSource` and ancestor-relative `reachableActivities`. These activities are possible source continuations, not produced results. The target body can be unentered even when its delegation action was evaluated and guarded out. Resolved cyclic targets carry `targetSource` without `deferred`; unresolved targets carry neither. `reachableActivities` is present only for deferred targets, may be empty, and contains bare activity names: a static, guard-blind union excluding delegation ancestors, not case-specific reachability. Descendant node IDs below unentered delegations are absent from the VM; entered occurrence IDs are unchanged. The static MV tree retains all Decision definitions. In both contracts an expected pause has `pause: true` instead of an activity `branch`. Unknown trace/explanation satisfaction is omitted, not false. Consumers must handle that union and optional satisfaction.
...
```bash
crl-emit --path features/.../cms22.cel                       # → tests/data/fhir/patient/…
crl-emit --path features/.../cms22.crl  --target fhir-def    # → src/cql/ + src/fhir/
```

`--out-dir` is optional; it names the ROOT to write under and defaults to the project root.
