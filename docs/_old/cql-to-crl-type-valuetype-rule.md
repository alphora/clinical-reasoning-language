...
> **⚠ §7 SUPERSEDED IN PART — decision B (2026-08-11), realized in the #189 grammar+validation slice.** §7's claim that "mixed-shape operands are legal under explicit author declaration; the validator MAY warn but MUST NOT block" is **no longer the shipped rule**. Read §7 subordinate to [CRL-NORTH-STAR.md](../CRL-NORTH-STAR.md) and [emit-consistency-189-design.md](emit-consistency-189-design.md) §7. Shipped (`src/validator/useSiteTypeValidator.ts`, commit `b1068ca`), the rule has **two cells**: (1) a leaf declaring `value type is boolean` inside a composition whose parent declares a **non-boolean value type** is a hard **ERROR** (`boolean-in-refinement-composition`, value-type-keyed, non-demotable) — fix by giving that leaf its resource value type, or declaring the parent boolean; **`exists` does NOT fix this** (it produces another boolean). (2) Any other result-type disagreement the implicit-existence bridge permits — a **boolean parent over a resource/record leaf**, two differing non-booleans, or a differing record resource — is a **WARNING** (`composition-result-type-mismatch`) today that **becomes an ERROR at the #189 flip**; fix the boolean-parent + record-leaf cell by making the bridge explicit with **`defined as exists ( "Record Concept" )`**. See [defined-as-is-semantic-composition.md](defined-as-is-semantic-composition.md) for the full banner.
...
> **⚠ SUPERSEDED for the existence case by the #189 emit-consistency model (`docs/CRL-NORTH-STAR.md` §3).** The rule above — "a computed boolean predicate over a non-native-boolean resource flips to `Observation+boolean`" — is NOT how a **boolean determination over a valueless resource** is now modeled. The canonical form keeps the **natural resource** and reduces it with existence: `- type is Condition. - value type is boolean. - code is \`x\`. - definition is exists this.` → `exists([Condition: "x"])`. Here `value type is boolean` names the **result of `exists`**, not a claimed `Condition` value element (Condition has none), so the "resource must have a native boolean field" constraint does **not** apply — it applies only to a representation read as a *value* (a stored boolean), never to the output of `exists`/`count`/an age calculation. Do **NOT** rewrite a `Condition`/`Encounter`/`Procedure` existence determination to `Observation+boolean`; that manufactures the wrong retrieve and breaks the local-domain round-trip. The `Observation+boolean` rule still holds for a genuinely *stored* boolean value (an actual `Observation.valueBoolean`). See North Star §3 (the "boolean from a valueless resource" example) and `docs/_old/emit-consistency-189-design.md` §2 (the `exists this` row).
...
**On inference operators (`sem-and`, `sem-or`, `sem-not`):** these are **SEMANTIC INFERENCE** operators (over ONE concept's representations/components — NOT decision composition, #168), not boolean logic. The author declares the resulting concept's `(type, valuetype)`; the sem-* operators describe HOW the meaning is inferred (intersection / union / exclusion at the semantic layer); they DO NOT type-check the operands against each other or against the result. Operands of mixed shapes compose only within guardrails (SUPERSEDED IN PART by decision B — see the §7 banner and [defined-as-is-semantic-composition.md](defined-as-is-semantic-composition.md)): a **boolean** leaf inside a **non-boolean** composition is a hard error (fix by giving the leaf its resource value type or making the parent boolean — NOT by adding `exists`), and a **boolean parent over a resource/record leaf** warns today (implicit `exists` bridge) and **errors at the #189 flip** — the author makes it explicit with `defined as exists ( "X" )`. This is still the "What not How" principle — authors declare WHAT a concept means; the implementation handles HOW — with the guardrails decision B added — where **HOW is the TARGET-language realization (which CQL expression, which FHIR element), NEVER a CRL operation the author could have written**. See [CRL-NORTH-STAR.md](../CRL-NORTH-STAR.md) §0.

**See [defined-as-is-semantic-composition.md](defined-as-is-semantic-composition.md)** for the full principle, worked examples, and the common mis-readings to avoid. That document is mandatory reading before authoring or auditing any CRL concept with `defined as` inference bodies.
...
**Inference operators in the chain.** When `T_C` and `V_C` are validated against a `defined as` inference body (`sem-and(...)` / `sem-or(...)` / `sem-not(...)`):

- **The author declares the result `(T_C, V_C)`; that declaration is authoritative.** The sem-* operators are SEMANTIC (intersection / union / exclusion of meaning), not boolean logic. They do NOT impose *element-for-element* type-matching on operands.
- **Mixed-shape operands compose only within guardrails** (SUPERSEDED — see the §7 banner). A **boolean** leaf inside a **non-boolean** composition is a hard **error** (`boolean-in-refinement-composition`) — fixed by giving that leaf its resource value type or declaring the parent boolean, **not** by an `exists` lift. Any other result-type disagreement the bridge permits — a boolean parent over a resource/record leaf, or two differing non-booleans — is a **warning** (`composition-result-type-mismatch`) today that **becomes an error at the #189 flip**.
- **For boolean-declared concepts**: a resource/record operand is bridged today via implicit `exists`, but the validator now **warns** that this bridge retires at the flip — make it explicit with `defined as exists ( "X" )`. This direction is on a migration deadline, not "unchanged."
- **For refinement-declared concepts**: the chain check applies to the SUBJECT (first concept ref in left-to-right traversal of the composition expression) — that subject's `(T_S, V_S)` must satisfy `T_C = T_S` AND `V_C = V_S`. Other **non-boolean** operands with a different result type (record resource, or shape) **warn** (`composition-result-type-mismatch`, error at the flip); a **boolean** operand under a non-boolean value-type parent is an **error** (`boolean-in-refinement-composition`), fixed by realigning the leaf's value type.
- **Heterogeneous-resource inference** (e.g. `sem-or` of a `ServiceRequest` refinement with a `MedicationRequest` refinement) is bridge-emitted in N by author declaration: the author picks an umbrella result type (typically `Observation+boolean` if no `DomainResource` umbrella exists yet) and the emitter unions / boolean-wraps each typed operand. **This is transitional (per the §7 banner): the differing operand result types warn (`composition-result-type-mismatch`) today and error at the #189 flip — reduce each operand to the common result type first (e.g. boolean via `defined as exists ( "X" )`).**
...
### 7.6 Future cross-check against CRLCommon.cql

Once todo 5 ships, each pattern function in CRLCommon.cql has a concrete return type. The validator gains a second consistency check: the function's return type must match the concept's `(T_C, V_C)` according to:
