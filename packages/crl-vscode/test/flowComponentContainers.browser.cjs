...
const flagFits=()=>[['inner',flag],['outer',outerFlag]].every(([key,n])=>{const b=frame(key),r=n.getBoundingClientRect();return Math.abs((r.left+r.right)/2-(b.right-10))<1&&Math.abs((r.top+r.bottom)/2-(b.bottom-9))<1;});
...
const result={flagAttached:flagFits(),nested:outer.right>inner.right&&outer.bottom>inner.bottom,enclosesCard:inner.bottom>card.bottom&&inner.left<card.left,clearSibling:inner.bottom<sibling.top,excludesOutcome:outer.right<end.left,controlsRetained:controls.every(n=>n.isConnected)};
f.querySelector('.route-layout-toggle').dispatchEvent(new MouseEvent('click',{bubbles:true}));result.detachedOutside=f.querySelector('.route-questionnaire').getBoundingClientRect().bottom<frame('outer').top;draw();draw();result.flagRepeatedDetached=flagFits();result.singleLayer=f.querySelectorAll('.flow-component-frames').length===1;
...
})()`,returnByValue:true});if(r.result.exceptionDetails)throw Error(JSON.stringify(r.result.exceptionDetails));const v=r.result.result.value;console.log(v);assert.ok(v.flagAttached&&v.flagRepeatedDetached&&v.flagReset&&v.flagTransformRestored);assert.ok(v.nested&&v.enclosesCard&&v.clearSibling&&v.excludesOutcome&&v.controlsRetained&&v.detachedOutside&&v.singleLayer&&v.collapsed);assert.equal(v.unpinnedFrames,2);
