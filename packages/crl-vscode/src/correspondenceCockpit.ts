...
export const COCKPIT_WEBVIEW_SCRIPT =
  ...
  `const targetLink=e.target.closest&&e.target.closest('[data-flow-open-decision]');if(targetLink){e.preventDefault();e.stopPropagation();const key=targetLink.dataset.flowOpenDecision;const target=[...root.querySelectorAll('[data-flow-key]')].find(n=>n.dataset.flowKey===key);if(target&&!target.classList.contains('flow-focus-hidden'))focusPinLeaf(key);return;}` +
  ...
  `document.addEventListener('keydown',e=>{if(e.defaultPrevented||(e.key!=='Enter'&&e.key!==' '))return;const link=e.target.closest&&e.target.closest('[data-flow-open-decision]');if(!link)return;e.preventDefault();link.dispatchEvent(new MouseEvent('click',{bubbles:true}));});` +
