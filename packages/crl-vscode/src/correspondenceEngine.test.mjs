...
check("select → a reveal for ALL visible panes (pane-agnostic, incl. crl/cel + the opt-in tree/questionnaire), targeting the unit", () => {
  ...
  // fhirQuestionnaire was MISSING from the engine's PANES when that pane was added, so it alone received no
  // reveal effects — it rendered but never highlighted the selection. Both questionnaire panes fan out now.
  assert.deepEqual(r.effects.map((e) => e.pane).sort(), ["cel", "crl", "fhirQuestionnaire", "questionnaire", "source", "tree", "worklist"]);
...
check("setPaneVisible suppresses that pane's reveal; re-showing with a selection reveals it", () => {
  ...
  assert.deepEqual(r.effects.map((e) => e.pane).sort(), ["crl", "fhirQuestionnaire", "questionnaire", "source", "tree", "worklist"]); // cel hidden; tree/both questionnaires/worklist still revealed
...
check("setInputs keeps + re-reveals a surviving selection; clears a gone one", () => {
  ...
  assert.equal(kept.effects.length, 7); // re-reveal all 7 visible panes (source/crl/cel/tree/questionnaire/fhirQuestionnaire/worklist)
...
check("setInputs keeps a surviving crl selection (+re-reveal), clears a vanished one", () => {
  ...
  assert.equal(kept.effects.length, 7);
