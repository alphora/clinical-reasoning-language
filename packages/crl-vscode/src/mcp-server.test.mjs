...
check("authoring_kit full exports all 19 artifacts and determination guidance", async () => {
    ...
    assert.equal(kit.schemaVersion, "2.16");
    assert.equal(kit.contentHash, "d5099f1b7a0b82af0b23b2ec2706afec50dd23799da6ad43b12426a92598fbd4");
...
check("bundled create_flag persists Description separately and preserves retry identity", async () => {
...
check("title creation survives persisted reload, legacy retry, and conflict rejection", async () => {
  ...
  assert.equal(readFileSync(file, "utf8"), before);
  const source = ts.createSourceFile("host.ts", readFileSync(resolve(here, "correspondenceCockpit.ts"), "utf8"), ts.ScriptTarget.Latest, true);
  let modelFunction;
