...
import { REUSED_CONDITION_SNIPPET, REUSED_CONDITION_CRL, REUSED_CONDITION_CEL, SHARED_CONTINUATION_CRL, SHARED_CONTINUATION_CEL } from "./decisionExamples";
import { CODED_SOURCE_POLICY } from "./codedSourceExample";
...
// "2.2" → "2.3": MV suite selection, regression runner, and conservative off-path advice.
// "2.3" → "2.4": construct roles, shared continuations, and explicit CRE expectation limits.
// schemaVersion → "2.5": native runtime reliability/retry, reached-pause correspondence, and durable concept flags.
// schemaVersion → "2.6": text answers and explicit selected-value presence; no collection architecture.
...
const CONCEPT_LAYER_MODEL: ConceptLayerEntry[] = [
  {
    ...
    meaning:
      "Declare value type explicitly on selected Observation publications: boolean for a finding used as a guard, Quantity for a measurement, CodeableConcept for a coded answer, text for a free-text answer (normalized to FHIR string). The Case Feature expression returns the selected record; supported guards read its Boolean value. Do not relabel coded or numeric data as boolean to satisfy a guard: derive a separate qualification, threshold or has-value publication. Missing answers remain unknown; record presence does not establish a true answer. Selected publications are rejected in legacy aliases, sem composition, collection-existence reductions and per-action menu guards (publication-unsupported-context). Use supported publication producers and branch/criterion guards. Validation checks do not replace emitted CQL and native execution.",
    scope: "in",
  },
  {
...
const RULES: KitRule[] = [
  {
    ...
    clauses: [
      { text: "Has-value tests only the selected publication's value. No selected record or a selected valueless/extension-only primitive record produces false. Valid text, Boolean false and recognized Unknown/N/A codes produce true. Supported operands are selected Boolean, text/string, dateTime and domain-interpreted CodeableConcept publications. Quantity and other complex presence contracts are not supported. Invalid type, unrecognized coded domain and selection failures remain errors. This predicate does not establish whether a diagnosis exists clinically or whether a dataset is complete.", force: "validator-enforced" },
      { text: "CEL rejects empty text values. Native inputs must be valid FHIR: the pinned FHIR parser can normalize an invalid empty valueString to an absent primitive before CQL evaluates it, so has-value is not a raw JSON schema validator. Clear an answer by retaining its response item without an answer, not by sending an empty string.", force: "default" },
      { text: "A selected operand supplies the computed record's validity and provenance; no operand yields an undated computed false with no invented clinical timestamp or evidence reference. A newer valueless record can replace an earlier answer. Clearing is a client/request contract, not removal of historical records: prove actual Q/QR extraction and selection with older answers still present. Omitting a field is not an instruction to clear it.", force: "default" },
      { text: "Do not use first: sibling presence guards to enumerate optional questions: the first true guard can stop later questions. The synthetic intake reference uses one compound presence guard and the same review activity in otherwise. It exposes the dependencies and recommends review even with incomplete answers; it adds neither requiredness nor a submission milestone. Author source-required conditional questions separately and test the applicable question set. An intake result is not a clinical eligibility finding.", force: "default" },
      { text: "Use value type is dateTime for temporal answers. It emits Observation.valueDateTime, Questionnaire type dateTime and QuestionnaireResponse valueDateTime using the standard engine. CEL value is contains the answer; date is timestamps the assertion for selection. Valid calendar-only values and zoned timestamps are accepted without inventing missing time. Renderer input/export behavior must be verified separately; a dateTime picker need not preserve date-only precision. Radio versus dropdown presentation and Record/RecordSet multiple-selection extensions remain backlog #322. Do not post-process emitted CQL, profiles or CEL data to invent unsupported language behavior.", force: "default" }
    ],
    ref: "docs/typed-intake-answers.md; src/emit/tests/publicationHasValue.test.ts; intake-reference.crl; intake-reference.cel"
  },
  {
    id: "concept-form",
    applicability: "All CRL authoring",
    category: "concept-model",
    rule: "For selected concepts declare shape is Record, type is Observation, value type is and shape reduction is most recent. Supply data with code is, a supported source representation or a supported definition is producer. code is provides the local answer representation; an uncoded calculation has no local answer slot. Coded answers use value domain is answer options and value from is \"Named Terminology\". Presentations separate questions from concept names. Supported patterns include Patient age, ServiceRequest existence, finite-code Quantity and CodeableConcept sources, answer qualification, thresholds and BMI with explicit validity. These are bounded contracts; unsupported composition is a capability gap, not a reason to copy legacy Scalar content. Missing evidence for an answerable determination stays unknown; false needs an explicit negative or a computation establishing it. For selected-datum membership, an interpretable known non-member yields false; no selected datum or value leaves it unknown. Membership of one selected datum is not existence of any matching record: a selected non-repair request cannot establish that no repair was requested elsewhere. Inferring a broader negative from absent records requires an explicit completeness assumption for subject, scope and time. The completeness language/enforcement remains #320 design work; neither code is nor receipt of a data bundle supplies that contract.",
  ...
  },
  {
    ...
    applicability: "All CRL authoring",
    category: "concept-model",
    rule: "Declare value type explicitly on selected Observation publications: boolean for a finding used as a guard, Quantity for a measurement, CodeableConcept for a coded answer, text for a free-text answer (normalized to FHIR string). The Case Feature expression returns the selected record; supported guards read its Boolean value. Do not relabel coded or numeric data as boolean to satisfy a guard: derive a separate qualification, threshold or has-value publication. Missing answers remain unknown; record presence does not establish a true answer. Selected publications are rejected in legacy aliases, sem composition, collection-existence reductions and per-action menu guards (publication-unsupported-context). Use supported publication producers and branch/criterion guards. Validation checks do not replace emitted CQL and native execution.",
  ...
  },
  {
    ...
    applicability: "All CRL authoring",
    category: "decision-shape",
    rule: "Choose structure from the source meaning. Alternative records of ONE clinical fact belong to that concept's supported source representations or producers; separate independently occurring facts used as policy criteria belong to decision logic. Do not invent a composition syntax when the required producer is unsupported: report the gap. DISTINCT criteria sharing a consequence can use a compound branch guard or a named criterion. Criteria routing to different consequences use separate when branches. Under first:, a false guard skips its branch, a true guard selects it, and a decisive unknown pauses; ordered siblings are not interchangeable with Boolean OR when an earlier operand is unknown. Full-conjunction guards permit later qualifying pathways when the earlier conjunction is false. A matched branch's descendants remain within that branch. Separate source-delegated or genuinely shared determinations may use use decision. Do not manufacture inference concepts merely to hide a policy's Boolean logic. Start with three responsibilities: a Concept provides a datum from supported answers, sources or computation; a Criterion defines the same Boolean condition once for reuse in multiple guards; a Decision gives conditions their source-required applicability, precedence and actions. Keep one-use conditions inline by default. Ordinary nested branches need no helper. An action-bearing continuation genuinely repeated within one decision can use a same-library helper without becoming a separate clinical determination; see chaining-necessity. Neither branch count nor criterion length determines fidelity. Vertical AND and horizontal AND are visual shorthand, not CRL constructs: distinguish ordered nested guards from operands in one compound guard. With A unknown and B false, nested A-then-B pauses at A, while A AND B is false; see guard-or-vs-sibling-or for worked references.",
  ...
  },
  {
    ...
    applicability: "All CRL authoring",
    category: "decision-shape",
    rule: "Use decision invokes action-bearing branch structure. Use it for a source-delegated determination, a genuinely shared determination, or a behavior-preserving internal continuation reached from several paths. A repeated remaining interview can be defined once in the same library and invoked from both callers; it need not have a separate clinical outcome or a name in the source. Preserve when it is reached, precedence, unknown/error behavior, action identity and multiplicity, and relevant question scope. Ordinary local nesting does not require a helper, and a Boolean condition reused in guards belongs in a criterion. Never couple independently owned policies merely because their criteria look alike. Current CRE publication preparation refuses foreign delegated decisions with publication-unsupported-scope; same-library continuations and imported concept operands are separate cases. Factoring existing CRL changes structural addresses and can change review attribution display: regenerate/check provenance and renew affected MV review; semantic equivalence does not mean identical node IDs. The worked inline-versus-shared comparison proves CRE activities and condition order only. Re-verify native action IDs, RequestGroup nesting and Questionnaire grouping for the actual policy; preservation is an obligation, not a guarantee supplied by factoring. Every helper emits an independently applicable PlanDefinition; no internal-only marker prevents direct invocation. Verify deployment and invocation paths expose the intended policy root, not the helper alone. For PA, also check the delegated closure under pa-disposition-set and verifyLoop:mutual-exclusivity-spans-closure, including callers that both delegate and recommend. Decisions contain branches, not bare top-level actions. Use a guarded action block for several roots only when the source has a real common applicability condition. Otherwise place independently invoked roots in separate policy packages (one unreferenced root per package); no verified unconditional action-only coordinator is supplied, and an always-true question must not be invented. See shared-continuation-reference for the small ordered example and source-delegated-decision-reference for a separate determination.",
    why: "Reuse should preserve source meaning and ownership. Factoring the same remaining actions differs from inventing an independent determination or coupling lookalike policies.",
    ref: "decision-composition; criterion; shared-continuation-reference; source-delegated-decision-reference; docs/decision-shapes.md",
    clauses: [
  {
    "text": "Use same-library helpers for genuinely repeated action-bearing continuations and use decision for source-delegated or genuinely shared determinations. Use inline nesting for local structure and criteria for conditions reused in guards.",
    "force": "default"
  },
  {
    "text": "An internal reuse helper must represent action-bearing structure genuinely reached from multiple caller paths; a single-use wrapper cannot claim this reuse rationale. Source-required separate determinations remain a distinct legitimate case. Do not present an internal helper as a new clinical determination. Preserve source applicability, precedence, unknown/error behavior, resulting actions and multiplicity, and question scope. Independently owned lookalike policies must not be coupled.",
    "force": "invariant",
    "test": "judgeLens.composition:invented-determination-boundary"
  },
  {
    "text": "Verify the intended root and each relevant caller path, including missing inputs and exact native output/question evidence. A passed single activity assertion alone does not prove that no extra actions fired.",
    "force": "invariant",
    "test": "verifyLoop:native-outcome-verification"
  }
],
  ...
  },
  {
    ...
    clauses: [
      ...
      {
        text: "Legacy inline DNF has an emitter materialization bound. A faithful model exceeding it is a capability gap, not an authoring-complexity violation. A criterion emits one named define and can reduce a legacy parent DNF. A faithful single-use factoring can be an explicit remedy for that compiler bound; report the gap as well and verify the resulting semantics. This exception does not change the default of naming for reuse. Whole-expression selected-publication guards already avoid this expansion. Single-use compiler-bound relief here is the Criterion remedy, not a reason to add a single-use Decision wrapper. Use a same-library continuation for actual multiple-caller reuse or a source-required delegated determination, preserving behavior and source ownership; never change ordering or invent a determination merely to satisfy an implementation bound.",
        force: "default",
      },
    ],
  },
  {
    ...
    ref: "artifact:reused-condition-reference.crl; docs/decision-shapes.md; validator rules criterion-cycle / criterion-misuse; #224",
    clauses: [
  {
  ...
  {
    "text": "Define the same Boolean condition once for reuse at multiple guards with the same meaning and scope. Keep one-use conditions inline; this is authoring guidance, not a validator usage-count rule. Preserve the condition and its answerable dependencies.",
    "force": "default"
  }
],
  },
  ...
  },
  {
    ...
    category: "cel",
    rule: "Prepare clinician-facing CEL cases to demonstrate the authored question routes, with only the data needed for the route being reviewed. Include earlier false answers required to reach a later branch; missing evidence is not No. Keep deliberate overlap, precedence, conflicting-evidence and pause/clear controls in engineering verification. Those controls remain necessary where required by cel-cases and native-outcome-verification; simplifying an MV demonstration must not remove their proof. Regression harnesses may reuse MV cases and add controls; CEL has no CEL-to-CEL fact import. Normal emit_cel, emit_results and MV discovery select the complete src/cel/mv suite, including when passed one MV file. Each policy needs its own package.json beside src, and every selected CEL file must cover the same CRL root. Unclassified CEL and explicit regression selections are refused by normal emission/MV. An existing empty mv folder means zero cases; a missing folder is a migration error. crl-run-regression evaluates MV plus regression once each without emitting; its --native option uses the existing native producer in an automatically created temporary directory (or retries that prior scratch run with --retry-from). Files have independent fact namespaces; CEL library names and frozen case IDs must be unique across the union, while case display names may repeat across files. Explicit validate_cel on MV checks the MV set; on regression it checks the union. Normal emission does not evaluate regression. Project-relative provenance references include src/cel/mv or src/cel/regression; basename aliases work only when unambiguous. FINAL case correspondence checks MV only. Data/results manifests retain each case source and frozen ID. Normal command results and native case states report failures. Explicit MV emit/validation commands may warn about supplied local Boolean question facts used only by definitively skipped later CRE conditions; unknown, computed, source/selector, criterion and potentially shared dependencies suppress this conservative advice. No warning is proof that all data is necessary. A deliberately retained clinical alternative can be valid despite advice; judge it against the intended demonstration instead of automatically deleting it. Warnings neither remove facts nor establish native equivalence. Use Git for source and review-history changes. Preserve frozen IDs for unchanged examples and check review associations when moving cases. Changed case data or meaning requires renewed review; use a new frozen ID or explicitly reset its prior verdict. Use Git to retain the old review file before removing obsolete regression-only entries from the current MV review file; those stale entries otherwise prevent MV completion. The pinned Result Questionnaire walks the selected executed route and its answerable inputs. Extra data for a skipped condition does not add that question to this route; parallel produced routes remain separately inspectable. The retained CRL Questionnaire covers the complete reached case rather than just the pinned route. The FHIR Questionnaire shows the associated native output. These three views serve different purposes and can differ legitimately. Displayed answer labels and choice highlights are not a terminology-validation oracle; validate coding identity and native behavior independently. Tree execution does not depend on authored source correspondence; source-text highlighting still does. Neither viewer repairs authored CEL or replaces native verification. Shared continuations retain their full source definitions in the tree; the case view expands actual entered occurrences instead of repeatedly copying every possible unentered path. Off-path advice follows CRE traces and conservatively keeps possible shared uses. A CRE error in any sibling case suppresses advice for that entire CEL file. It remains advice, not a completeness proof. Keep assertion stress controls in regression; worked reference files marked engineering are not ready-made minimal MV suites.",
    ...
    ref: "cel-cases; verify-loop; docs/cel-suites.md; docs/mv-component-view.md; docs/medical-validation-plan.md; crl-vscode/src/executionRoutes.test.mjs",
    clauses: [
  ...
  },
  {
    ...
    clauses: [
      {
        "text": "Collisions in actual emitted resource paths, within a case or with a previously emitted case in the same run, report id-collision as an error and prevent the colliding case from emitting. Normal MV publication refuses the complete selected suite when any case has an error or cannot emit; it does not publish a partial demonstration set. Different fact names can collide after normalization; repeated Patient references are exempt, and the ambient Encounter participates. Reusing a fact across separate cases is legal when their emitted paths remain distinct.",
        "force": "validator-enforced"
      },
      {
  ...
  },
  {
    ...
    clauses: [
      ...
      {
        "text": "Verify that the selected root and complete returned paths are intended before publishing. Select a root above the documented generated layout, not a leaf that duplicates src or tests/data/fhir below itself. Account for complete CRL/CEL/results replacement, and verify the emitted path set after renames.",
        "force": "invariant",
        "test": "verifyLoop:artifact-integrity"
      }
    ],
  ...
  },
  {
    ...
    applicability: "All CRL authoring",
    category: "process",
    rule: "Emitted trees are producer-owned. After complete preflight, CRL replaces src/cql and src/fhir, standalone CQL replaces src/cql, CEL replaces tests/data/fhir, and normal native results replace tests/results. Every old or custom file in those generated directories is removed; keep authored files elsewhere and use Git for recovery. Explicit failed-case retry retains verified successful outputs and retries the rest. Linked output boundaries are refused; links within a removed tree are never followed. Failed validation leaves prior output intact; a filesystem failure after replacement begins can leave partial output and is reported. CEL and native results write case manifests with paths and hashes. Re-hashing listed files proves integrity, not completeness: compare the complete path set too. Consume returned paths and manifests.",
    ref: "verifyLoop",
    clauses: [
      {
        "text": "Keep authored files outside producer-owned data/results trees. Compare the complete expected path/resource set with returned manifests as well as file hashes; inspect replacement, explicit-retry retention, symlink and removal diagnostics. Integrity of listed files alone does not prove completeness or absence of stale output.",
        "force": "invariant",
        "test": "verifyLoop:artifact-integrity"
      }
    ],
  ...
  {
    id: "fhir-packaging",
    applicability: "Distributing existing FHIR definitions as an NPM package",
    category: "process",
    rule: "For a definitions-only FHIR collection Bundle, call emit_crl_bundle with an absolute CRL path, or SDK emitCrlBundle(path). It emits the dependency closure and embeds local CQL in Library.content.data without writing files or adding patient data or a static Questionnaire. Inspect success and diagnostics; emission errors, unresolved references, filename collisions and missing CQL refuse the Bundle. External dependencies such as hl7.fhir.uv.cql.FHIRHelpers still need runtime resolution. This is not native execution or FHIR/profile validation. After emitting and verifying definitions, call package_fhir with an absolute projectRoot, or crl-package-fhir --project <path>. This packages existing src/fhir; it does not emit, publish, or run $apply. Configure package.json name (the policy ID), exact SemVer version, author, description and crl.canonicalBase, plus a separate dotted crl.packageId (at most 64 characters) and exact crl.fhirDependencies for external FHIR packages. packageId/--package-id can override the package ID. The R4 core dependency is 4.0.1. An optional license must be a single FHIR R4 SPDX code; no license is invented. The default output/package.tgz contains package/package.json, an ImplementationGuide resource inventory, a v2 .index.json and the canonical definitions. Existing resource identities and versions are preserved. Local Library attachments within src are embedded only in the archive copy; source files are unchanged. Only the chosen archive is replaced. Optional outputFile/--out selects its destination outside src. Patient examples and MV/regression data are refused under src/fhir. Duplicate identities, unsafe paths and invalid metadata are errors. Identical inputs produce identical archive bytes. Known CPG, CRMI, SDC, CQL and terminology dependencies are checked, but packaging does not resolve transitive dependencies or prove FHIR/profile/CQL executable validity. Inspect the archive's package/.index.json and ImplementationGuide inventory and independently validate content for its intended use. A separate CRMI asset-collection manifest Library is not required merely to create an NPM package; this tool does not implement a server $package operation.",
    ref: "docs/fhir-npm-packaging.md; fhir-package/index.test.ts",
    clauses: [
      { text: "Provide valid package metadata and keep patient/case data outside src/fhir; the packager refuses invalid metadata, duplicate identities and unsafe paths.", force: "validator-enforced" },
      { text: "Verify emitted definitions before packaging and independently validate the resulting package for its intended consumer; packaging success is not executable or clinical validation.", force: "default" },
    ],
  },
  {
    id: "native-apply-session",
    applicability: "Applications submitting successive native QuestionnaireResponse edits",
    category: "process",
    rule: "Use @smile-digital-health/crl/session applySession or crl-apply-session for one stateless native R4 $r5.apply request. Supply schemaVersion 1, request/case/step IDs, PlanDefinition ID, subject, immutable repository JSON and explicit current request-data JSON; the CLI accepts JSON file paths instead. Include the matching returned Questionnaire with a submitted response. buildSessionResponse requires the exact response SHA256, authored time and mode: full reasserts supplied contents; edits-only submits explicit edits and ancestors, omitting untouched answers. Set native value[x] answers; clear retains an item without its answer. False is a value. No date coercion or timestamp inference occurs. Edits-only under repeating groups or answer children is refused; full mode preserves supplied hierarchy but does not establish persistence identity. The caller owns resource replacement, retained extracted state and acceptance of successful results; no hidden state or universal merge rule exists. Error/fatal native outcomes, invalid inputs, process failures, timeout, cancellation, output limits or unconfirmed cleanup fail the request. Raw input/output artifacts, hashes and runtime identity remain available. A returned Questionnaire or ok:true alone does not prove a clinical disposition; inspect expected actions and answers. Use JRE17+ and the pinned engine. See packages/crl/docs/apply-session.md for the exact API and limits. Native engine question structure is consumed as returned, not repaired by this adapter.",
    ref: "packages/crl/docs/apply-session.md; results/tests/sessionResponse.test.ts; results/tests/session.test.ts",
    clauses: [
      { text: "Choose full or edits-only explicitly and distinguish omission from clear; retain prior successful caller state on failure.", force: "default" },
      { text: "Verify native actions, answers and errors independently from adapter success; repeated-resource reconciliation remains caller-owned.", force: "default" },
    ],
  },
  {
    id: "produce-results",
    applicability: "All CRL authoring",
    category: "process",
    rule: "Questionnaires are generated by `$apply`, never emitted from CRL: no Questionnaire appears in the CRL definition emit. After validation and CRE checks, call `emit_results(celPath, crlPath, useCase)` (useCase selects result-driver behavior, not kit content); it owns result placement, so never place `$apply` output yourself. Enable `crl.enableResults` in VS Code User settings and restart the MCP client. An absent setting preserves an existing opt-in; explicit false removes it. A JRE 17+ and the CRL-maintained complete CLI engine " + ENGINE_JAR_SOURCE.buildId + " are required. The tool gives the download command when missing, discovers <home>/" + ENGINE_JAR_SOURCE.cacheRelativePath + ", and verifies the pinned hash; normal use needs no manual hashing, extraction, or classpath. The original Maven jar is not the corrected default. `jarPath` and `jarSha256` are overrides. Normal emit_results selects the complete MV suite. Its schema-1 manifest is tests/results/questionnaire-manifest-mv.json; celLibrary labels the selected set and each entry retains sourceFile and caseId. Read every case state: generated / no-questionnaire / populate-degraded / failed / timeout / not-run. The unsuccessful-case count includes degradation and not-run while preserving their distinct states. A clean no-questionnaire result must be compared with the particular case expectations; inputs elsewhere in the definition closure do not prove this route should have a form. Normal runs replace the complete generated results directory, including old manifests, after source/runtime preflight. Custom files there are deleted; Git owns recovery. An absent file alone is not evidence that the policy asked nothing. generated means a Questionnaire was returned, not that the intended activity or pause was verified; compare those outcomes against each case oracle. Written MV pairs have stable per-case identities and omit QuestionnaireResponse.authored to avoid run-time churn. They are MV review artifacts; do not resubmit these normalized files unchanged as interactive responses. The results tree is producer-owned; use returned paths/manifests and keep hand-authored Q/QR elsewhere. Set caseTimeoutMs (CLI --case-timeout-ms) to increase the per-case deadline; default 600000 milliseconds. Native runs own their subprocesses and allow one active producer per server. On Windows they require Windows 10/Server 2016 or later and Windows PowerShell. If cleanup is unconfirmed, inspect the reported processes and restart that server before another native run. Output preserves UTF-8 question and answer text. Use retryFailed: true (CLI --retry-failed) to retain compatible successful results and rerun unsuccessful, changed, or damaged cases. A changed model or runtime, or an older manifest without compatibility fingerprints, requires a full run. Retry replays the original CEL input clock; retained rows are historical results with their original producedAt, while reruns evaluate at the live engine time. generatedAt identifies the current invocation and reused marks retained rows. For a fresh evaluation of all cases, run without retry. Native regression may retry its previous scratch run with --retry-from <manifest>; it cannot target MV output.",
    ref: "verifyLoop",
    clauses: [
      {
        "text": "Use emit_results after validation and CRE checks, enabling it through crl.enableResults in VS Code User settings and restarting the MCP client, and using the maintained verified engine. Consume returned paths and manifests; use explicit jar overrides only for a deliberate alternate engine. Use caseTimeoutMs for a larger per-case native deadline and retryFailed to retain compatible historical successes. Inspect reused and producedAt; retry is not a fresh evaluation of every case. Changed model/runtime requires a full run. Use regression --retry-from only for the earlier regression scratch manifest.",
        "force": "default"
      },
      {
  ...
  },
  {
    ...
    rule:
      ...
      "For concept flags, create_flag captures an authored `- meta is `@id: stable-concept-id`.` as anchor.entityId. Keep that unique ID when renaming or moving the same concept; tree badges, folded Criterion rollups and flag navigation resolve its current location without rewriting the saved anchor/history. A supplied ID that is absent or duplicated never falls back to a reused name. Older flags without entityId still require a unique library/name match and do not automatically gain rename safety. Empty or repeated target IDs are refused when creating a flag; malformed identity metadata prevents reliable ID resolution. Generated dedup keys use the stable ID when present; explicit supplied keys remain unchanged. After a rename, inspect unresolved flags and their original evidence; do not assign a different concept the old ID merely to move a flag. " +
    ...
    clauses: [
      {
        text:
          ...
          "open`), with a short title in `gist` and the source detail, reasoning, consequences and review question in `description`; an optional tracker issue can be linked with `ref`. (`assumption`/" +
      ...
      },
      {
        text:
          ...
          "(a same-content retry returns the existing record; Description is excluded from generated keys and does not overwrite existing detail on retry; explicit keys remain caller-owned). PRECONDITION: the store is located by walking up to the " +
...
const TYPE_ALLOWLIST: TypeAllowlist = {
  ...
  note:
    ...
    "text (FHIR string), dateTime, CodeableConcept or Quantity values. Patient and ServiceRequest can be source " +
...
const EXAMPLES: KitExample[] = [
  { id: "datetime-answer", title: "Date/time answer", language: "crl", valid: true, snippet: DATETIME_ANSWER, note: "Selected Observation dateTime answer. Add to a CRL library with crl.canonicalBase for emission; pair with a has-value predicate when a Boolean guard is needed. Native intake acceptance verifies the actual generated dateTime question, calendar precision, timestamps and clear. Client picker behavior is a separate boundary." },
  ...
  { id: "coded-observation-source", title: "Selected coded Observation source", language: "crl", valid: true, snippet: CODED_SOURCE_POLICY, note: "Complete synthetic source-only policy shared with the coded-source emitter and CRE tests. Declare the tested source and result terminology; do not replace it with clinical codes without source justification. Supply package.json name coded-source and crl.canonicalBase https://example.org/coded-source. No local code means no editable question. Interpretation follows final selection; no extra indeterminate modifier is introduced. These source tests do not certify a customer policy or native execution." },
  ...
  },
  {
    ...
    language: "crl",
    snippet: REUSED_CONDITION_SNIPPET,
    valid: true,
    note: "Fictional source: every in-scope request is single or combined. Resolve that request type first; then either finding qualifies, and only combined requests need documentation. Qualifying Evidence is reused twice. The four inputs are selected Boolean Record/Observation concepts, not presence tests; missing is unknown. Complete declarations and engineering cases are in reused-condition-reference. Other or overlapping request categories need their own source-faithful model. Neither this excerpt nor its CRE checks certify native question visibility.",
  },
  {
...
const VERIFY_LOOP_NOTE_BASE =
  ...
  "REPLACES the generated `tests/results/` directory on normal runs — do not hand-author artifacts there. " +
  ...
  'clusterBy:"disposition-path" — it clusters per grounded RUN PATH (decision-node refs only). Correspondence requires frozen, uniquely identified cases, successful rendering and grounded execution paths. Reached unknown conditions contribute pause endpoints; produced actions contribute action endpoints. A partial path with both retains both and is labeled Awaiting input. A pause expectation alone is not execution evidence: no-output runs without a reached unknown, unresolved entered decisions, evaluation errors and discarded unknown evidence remain unchecked. Check generation diagnostics and FINAL validate_provenance results; generation success alone is insufficient. The check compares actual CRE paths with cockpit lighting and detects missing or extra rows, including on pause paths. It neither proves native equivalence nor completes source attribution. Keep useful pause tests; never invent a disposition to clear a diagnostic. ' +
  ...
  composition: [
    {
      ...
      weightedBy:
        "whether the use preserves a source-delegated determination, genuinely shared determination, or repeated internal continuation.",
      guidance:
        "A use decision is faithful for source-delegation OR a genuinely shared determination, and can also factor a behavior-preserving internal action-bearing continuation. An internal helper need not yield its own separate clinical determination. Reuse alone does not establish shared clinical identity: independent policies with coincidentally similar criteria may diverge and must not be coupled. Check source meaning, ownership, applicability, order, unknown/error behavior, actions/multiplicity and question scope.",
      checkpoints: ["Is this source-delegated/shared determination, or is the internal continuation genuinely reached from multiple caller paths rather than a single-use wrapper?", "Does factoring preserve the callers and resulting behavior, including unknowns and native questions?", "Is a condition-only reuse better represented as a criterion?", "Would factoring independent lookalike policies create false coupling?"],
    },
    {
...
const REFERENCE_ARTIFACTS: ReferenceArtifact[] = ([
  ...
  {
    name: "reused-condition-reference.crl",
    language: "crl",
    applicability: "Learning reusable conditions and source-ordered continuations",
    purpose: "Fictional request universe: single or combined only. Resolve request type first. Both paths reuse Finding A OR Finding B; combined additionally requires documentation in the same guard. False documentation can decide failure despite unknown findings. Concepts use selected Boolean Observations; Criterion is not an extra answer. Question presentations are included. No native or clinical-source certification. The owning test checks exact activities and distinguishes the single and combined branches; the CEL activity assertion alone checks membership.",
    verification: ["cre-run", "fhir-emit"],
    source: REUSED_CONDITION_CRL,
  },
  {
    name: "reused-condition-reference.cel",
    language: "cel",
    applicability: "Learning reusable conditions and source-ordered continuations",
    purpose: "Engineering regression reference, including explicit missing-input controls. Use one reference pair per separate scratch project. Place under src/cel/regression with the companion CRL under src/crl and the served requires.crl configuration in package.json. Create src/cel/mv too; it may be empty for these standalone engineering examples. Validate the CEL, then run crl-run-regression --project <project>. Reuse selected path demonstrations separately for MV; do not copy this complete stress set into MV. Exact CRE outputs are checked; native outcomes and interactive answer/change/clear behavior require separate verification.",
    verification: ["cre-run"],
    source: REUSED_CONDITION_CEL,
  },
  {
    name: "shared-continuation-reference.crl",
    language: "crl",
    applicability: "Learning reusable conditions and source-ordered continuations",
    purpose: "Fictional ordered source: resolve C0 (does extra evidence apply?) first, then D0 (is it sufficient?) only if applicable, then E (remaining check). Step1 is the same remaining branch/action structure used by both paths. C0 unknown/E false and C0 true/D0 unknown/E false both pause: this is deliberately not the flat formula (not C0 or D0) and E. Schematic names keep the structure small. Question presentations are included. No native or clinical-source certification. The owning test checks exact activities and entered condition paths; the CEL activity assertion alone checks membership.",
    verification: ["cre-run", "fhir-emit"],
    source: SHARED_CONTINUATION_CRL,
  },
  {
    name: "shared-continuation-reference.cel",
    language: "cel",
    applicability: "Learning reusable conditions and source-ordered continuations",
    purpose: "Engineering regression reference, including explicit missing-input controls. Use one reference pair per separate scratch project. Place under src/cel/regression with the companion CRL under src/crl and the served requires.crl configuration in package.json. Create src/cel/mv too; it may be empty for these standalone engineering examples. Validate the CEL, then run crl-run-regression --project <project>. Reuse selected path demonstrations separately for MV; do not copy this complete stress set into MV. Exact CRE outputs are checked; native outcomes and interactive answer/change/clear behavior require separate verification.",
    verification: ["cre-run"],
    source: SHARED_CONTINUATION_CEL,
  },
  {
