...
const CONCEPT_LAYER_MODEL: ConceptLayerEntry[] = [
  {
    ...
    meaning:
      "REQUIRED on every SCALAR concept — the DEFAULT shape and the Stage-1 default shape (A.10 `missing-value-type` is a validator ERROR for a Scalar concept; the requirement is now SHAPE-CONDITIONAL — a `shape is Record | RecordSet` concept, which is outside the introductory Scalar examples, MAY omit it and takes its result type from `type is`, see the `shape is` entry below) — an ORTHOGONAL property, NOT a producer or a layer: the concept's ONE canonical PUBLISHED result shape, the thing every use site consumes and type-checks against. Choose by what the concept RESULTS IN, not its FHIR resource type: boolean = a determination/finding (incl. ANY concept a decision/`criterion`/action guard consumes); Quantity = a measurement/value (most-recent-able); CodeableConcept = a coded refinement; dateTime/integer/string = a scalar datum. The value-type line does NOT itself produce a value — at least one PRODUCER from the entries below is also required (`x + n ≥ 1`; a value type with nothing producing it is invalid, #202). Composition: `concept = value type + ( n primitives and/or ≤1 derived )`. See rule value-type.",
    scope: "in",
  },
  {
    form: "- shape is Scalar | Record | RecordSet.",
    meaning:
      "The declared published shape: Scalar publishes a value, Record one selected record, RecordSet a collection. Scalar is the default; it declares `value type is`. Record/RecordSet obtain their result type from `type is` or a validated derived operand. These are language forms, not an Observation-only guard rule. FORWARD-LOOKING MODEL PRACTICE (outside these introductory examples): prefer reducing to a Record early when the clinical model leaves that choice: only the selected record then needs the boundary transform. Keep RecordSet when downstream logic genuinely consumes the history; a locally coded RecordSet is answerable. This is a practice, never a legality restriction. CURRENT LIMITS: a non-Scalar shape marker without a reduction warns `shape-marker-not-emit-active`; Record without a yielding definition errors `record-shape-invariant`. Local-code reductions over named operands or with source representations can fail `emit-reduction-not-active`; supported own-record reductions include `exists this`, `count this at least N`, and supported Scalar-boolean/Record `most recent this`. Verify the actual form through emit and the engine: #311 still affects equivalent most-recent forms, so this release does not mandate a bulk shape migration.",
    scope: "out",
  },
  {
    form: "- code is `local-code`.",
    meaning:
      "The LOCAL domain code is the canonical production representation; source representations provide additional evidence. In 4.121.0 write `type is` beside a local `code is`: omission is `local-code-missing-type`. A pure question is Scalar + Observation + boolean + local code, with no derivation or source representation. Its declared answer representation supplies the newest-answer read: true / false / unknown. It is EXEMPT from `no-bare-scalar-code`; adding `definition is exists this` changes the question into a presence computation and destroys its pause. A non-Observation record has a different meaning: `type is Condition` + `value type is boolean` + local `code is` + `definition is exists this` is a legal, implemented existence derivation over its own records, not a boolean answer slot. It belongs outside the introductory question examples; do not change a record derivation into an Observation question. Other bare Scalar forms may still warn; do not mechanically apply a migration warning to working question content. The age projection supplies its own reduction too.",
    scope: "in",
  },
  {
    form: '- source representation: - type is <Resource>. - [ coded from "Value Set" ] - [ value projection is <phrase> ].',
    meaning:
      "A source representation declares its resource type and optional `coded from` membership and `value projection`. The canonical datum carrier is MODEL INFO (`fhir-model/fhirValueModel.ts`), and the published value type belongs to the CONCEPT. Do not restate `value element is` or `value type is` on the representation: Condition.code yields a CodeableConcept, so declaring it boolean to describe existence says something false. Membership chooses source records; it does not offer answers. A projection computes from its own datum. This introductory kit works the Patient `age today <at least|at most|under|younger than> <N> years|months` family; unsupported projection phrases may parse without an emit implementation, so validate and execute the actual form. External record representations are supported in the language, including the request ServiceRequest; they are outside this kit's introductory local-only examples, not forbidden. Source evidence is additive to the canonical local determination.",
    scope: "out",
  },
  {
    form: "- defined as ( ... sem-and / sem-or / sem-not ... ).",
    meaning:
      "INFERENCE / semantic normalization: combines the sub-representations or data-components of ONE concept into ONE clinical fact (the only added depth this stage; #126). It is NOT decision composition and NEVER combines distinct decision criteria — that is the decision tree's job (#168). THE TELL (anchor the unit OUTSIDE the label): name the ONE clinical reality the operands each RECORD without using the concept's own name — alternative records of a SINGLE underlying occurrence (a viral-load lab result and/or a chart note attesting the SAME suppression — the records may themselves coexist, that is fine) are one fact. Operands that are SEPARATE underlying events, each independently occurring (a patient can fail drug therapy AND, separately, physical therapy), are DISTINCT criteria → decision layer, NOT `defined as`. The tell is SAME occurrence vs DIFFERENT occurrences, not whether the records coexist. run_decision evaluates it: sem-and = all, sem-or = any, sem-not = not. The MODEL requires unknown-preserving boolean composition. CURRENT CRE LIMITATION in 4.121.0: `defined as` composition converts unknown operands to false; a green CRE result is not proof of unknown propagation through that composition. Branch and criterion guards preserve unknown. Report lane disagreements and verify the engine result; do not encode around them. Bare operands resolve within the defining library; cross-library operands must be qualified. VALUE-PRESERVING: `sem-or`/`sem-and` and a bare-ref alias PRESERVE the concept's declared value type (the author declares it, the composition reconciles operands) — only a TOP-LEVEL `sem-not` and the `defined as exists (…)` sub-form are inherently boolean (see rule value-type). `defined as exists ( \"Concept\" )` tests record existence (present → true, absent → false). LANE MATRIX in 4.121.0: standard CQL and the CRE support record-existence reach-through (#270); this does not prove arbitrary scalar reductions, recency arbitration, or questionnaire answerability. Use an instance-bearing operand: exists over a scalar boolean can become silently always true through CQL list promotion. Emit rejects recognized scalar-boolean operands (#269); validate and execute the actual project, including cross-library forms. #317/#318 remain unresolved for computed boolean questionnaire items; a green CRE run does not establish a working question.",
    scope: "in",
  },
  {
...
const RULES: KitRule[] = [
  {
    ...
    clauses: [
      {
        text: "A Stage-1 leaf concept declares `value type is` (REQUIRED, a property present on the concept — NOT position-bound; the exemplars order it after `type is`) + `type is` + `code is` (local). The IN-stage PRODUCERS are `code is` (local rep) and `defined as` (inference); EXTERNAL `source representation`/`coded from` (value-set posreps) and `definition is` predicates are OUT this stage (the SOLE exception: the patient-age Patient `source representation` with a `value projection is age today <at least | at most | under | younger than> <N> years|months` — standalone or with a local `code is` override — see rule patient-age-projection). The general model `concept = declared shape + shape-appropriate result typing + ( n primitives and/or ≤1 derived )` holds package-wide: a Scalar concept (the default Stage-1 shape) declares a concept-level `value type`; a `shape is Record | RecordSet` concept (outside the introductory Scalar examples) takes its result type from `type is`. The introductory examples use Scalar concepts and therefore declare value type. Record/RecordSet forms remain legal; the shape-is entry states the boundary practice and #311 limit.",
        force: "default",
      },
      {
  ...
  },
  {
    ...
    clauses: [
      {
        text: "`value type` is REQUIRED on every SCALAR concept (the default Stage-1 shape; A.10 `missing-value-type` ERROR for Scalar — SHAPE-CONDITIONAL, a `shape is Record | RecordSet` concept may omit it and takes its result type from `type is`) and needs at least one producer (`x + n ≥ 1`, #202).",
  ...
  },
  {
    ...
    edge: "cpg",
    category: "guards",
    rule: 'A `when` BRANCH condition is a boolean over concept (and `criterion`) references — `and`, `or`, `not`, parentheses (CRL #224). A single ref needs no parens; a HOMOGENEOUS chain (`A and B and C` or `A or B or C`) may be bare; a MIXED `and`/`or` MUST be parenthesized (`( A or B ) and C`) — a bare mixed chain is a builder error. NEGATION is first-class: `not X` (`when not X`, `when A and not B`) and `not ( <compound> )` (parens required over a compound operand). STRONG KLEENE: `not unknown` is unknown; a negated branch does not turn an unanswered question into false. The guard STRUCTURE itself never collapses to one opaque CQL boolean: `and` → several ANDed applicability `condition[]` on one action; `or` → DNF arms (contiguous ordered siblings under `first:`; one `cqf-applicabilityBehavior:"any"` group under `all:`/flat); `not` → De Morgan pushes it to signed leaf literals, and a NEGATED literal emits a per-atom `not <ref>` applicability `condition[]`. Each ATOM — positive or negated — stays a VISIBLE `condition[]` / cockpit guard-box row: a CONCEPT atom is one `text/cql-identifier` naming the concept; a CRITERION atom is one `condition[]` naming the criterion (positive → `text/cql-identifier`; negated → `not <ref>` `text/cql-expression`), resolving to that criterion\'s OWN named boolean define (its sub-atoms visible in the transparent define body + the use-site `input[]` + an expandable named cockpit view-model node, post-#236) — NOT the parent guard collapsed. This visible-atom property is what separates a branch guard from `defined as` inference (which collapses distinct criteria to ONE opaque CQL boolean asserting a false sameness). DIFFERENT construct from the per-action guard (`unless`/`only when` on a menu member; see guards).',
    ...
    clauses: [
      ...
      {
        text: "A `when` branch condition is an `and`/`or`/`not` boolean over concept/criterion refs; the guard STRUCTURE lowers to PlanDefinition.action shape (ANDed `condition[]` for `and`; DNF arms for `or`; a per-atom `not <ref>` applicability `condition[]` for a negated literal) and never collapses to one opaque CQL boolean — so each ATOM stays a VISIBLE `condition[]`: an inline CONCEPT atom is its own action-level condition; a CRITERION atom is one identifier (positive) or `not <ref>` (negated) condition resolving to that criterion's named define, its sub-atoms in the define body + use-site `input[]` (post-#236, NOT the parent collapsed). Branch `not` lowers to FHIR emit as of #224 iii.3 (strong Kleene: `not unknown` is unknown), alongside the per-action `unless` (#224 iii.1 — see guards). A per-action `unless` is menu-member-only; a single-determination `first:` exclusion needs branch `not`.",
        force: "default",
      },
      {
  ...
  },
  {
    ...
    edge: "cpg",
    category: "cel",
    rule: "Author a companion `.cel`: `covers \"<CRL library>\"`; a Patient subject `fact` (`- defined by \"Patient\".`); one clinical `fact` per case-feature linked to its concept via `- defined by \"<library>\".\"<concept>\".`; and one `case` per path with `- subject is …`, the relevant `- fact is …`, and a `- result is \"<decision>\" is \"<branch>\".` oracle. The CRE resolves concept-linked facts and checks code membership for supported representations; an explicit code must match the resolved representation set. CURRENT LIMIT: when membership cannot be derived, the CRE falls back to name-based presence. This fallback violates the model and a green run does not prove that every code was checked. A bare concept-linked fact uses its declared local code. For a value-reading boolean question, write `value is true` or `value is false`; omission is UNKNOWN and pauses, not an implicit no. There is no absence code. Existence over records is different: no matching record gives false, and a value on a presence-only fact does not change its existence. A bare-type CEL fact without a code cannot match a coded retrieve; 4.121.0 emits warning `bare-type-fact-uncoded` for those resource types (#312). Patient is exempt because its retrieve is not code-scoped.",
  ...
  },
  {
    ...
    edge: "cpg",
    category: "process",
    rule: "Questionnaires are generated by `$apply`, never emitted from CRL: no Questionnaire appears in the CRL definition emit. After the verify loop, call `emit_results(celPath, crlPath, useCase)`; it owns result placement, so never place `$apply` output yourself. Enable `crl.enableResults` in VS Code User settings and restart the MCP client. An absent setting preserves an existing opt-in; explicit false removes it. A JRE 17+ and the supported cqf-fhir-cr-cli 4.7.0 jar are required. The tool gives the download command when missing, discovers its standard Maven location, and verifies the pinned hash; normal use needs no manual hashing, extraction, or classpath. `jarPath` and `jarSha256` are overrides. Read every case state: generated / no-questionnaire / populate-degraded / failed / timeout / not-run. An absent file alone is not evidence that the policy asked nothing. The results tree is producer-owned; use returned paths/manifests and keep hand-authored Q/QR elsewhere.",
    ref: "verifyLoop",
  },
  {
    id: "review-flags",
...
const VERIFY_LOOP_NOTE_BASE =
  ...
  "PROOF STATUS IS ORTHOGONAL TO FAITHFULNESS (§4): faithfulness decides the model, provability decides whether run_decision can prove it yet. Encode the FAITHFUL model and DEFER the proof for any construct the kit `boundary` marks out-of-scope — never substitute a less-faithful provable model, and never assert a composite to fake green (K4). Read artifact proof from `verification` and `verificationLegend`; scope in/out describes this introductory kit's coverage, not a language legality check or an execution verdict. Validate and execute forms outside the worked examples rather than assuming all predicates are deferred. " +
...
const VERIFICATION_LEGEND: VerificationLegendEntry[] = [
  {
    tier: "fhir-emit",
    means:
      "This exact CRL artifact is emitted by the kit suite every build with success, no hard errors, and at least one case-feature StructureDefinition.",
    doesNotProve:
  ...
  },
  {
    ...
    doesNotProve:
      "Membership only: it does NOT prove exact output, that a guarded item is ABSENT, or path identity (two cases ending in the same disposition are indistinguishable). Nor clinical `code is` correctness, engine retrieval, FHIR emit, or `$apply`. CRE code-membership execution is not a clinical terminology review.",
  },
  {
    tier: "engine-run",
    means:
