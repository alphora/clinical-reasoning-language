...
const RULES: KitRule[] = [
  ...
  },
  {
    ...
    clauses: [
      {
        text: "The RECENCY (local `code is` + age posrep) concept's EFFECTIVE local type is Observation — the recency merge emits an Observation-boolean retrieve. Authoring MAY omit the concept-level `- type is Observation.` under the implicit standard (the served exemplar does exactly this: `code is` + a Patient age posrep, no concept-level `type is`). A STANDALONE age posrep's own `type is` is `Patient` (the projected carrier).",
        force: "invariant",
