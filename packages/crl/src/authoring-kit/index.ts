...
const CONCEPT_LAYER_MODEL: ConceptLayerEntry[] = [
  ...
  {
    form: '- source representation: - type is <Resource>. - value element is <Resource.path>. - value type is <shape>. - ( coded from "Value Set" | value projection is <phrase> ).',
    meaning:
      "Query to an EXTERNAL source / value set — a fully-explicit SELF-DESCRIBING posrep (its own type + value element + value type). A posrep is NEVER terse (unlike the local rep) and, though it IS a producer (a primitive counting toward `x + n ≥ 1`), its own `value type is` does NOT by itself discharge the concept-level `value type is` requirement (A.10). The rep's producer line is either `coded from` (assert the datum against a value set — external, OUT of Stage-1 authoring) OR `value projection is <phrase>` — a rep-level COMPUTATION that TRANSFORMS the rep's own datum into the concept result. `value projection` is a bounded catalog: TODAY the sole built projection is `age today <at least|at most|under|younger than> <N> years|months` over `Patient.birthDate` (the IN-stage patient-age form — see below); a projection that resolves to no built override is REJECTED at the catalog boundary. Anchored `age at start of \"<anchor>\"` references ANOTHER concept, so it is a concept-level `definition is`, NOT a rep-local projection. Asserted (external value-set) posreps stay OUT of Stage-1 authoring; the Patient-age projection is the one sanctioned Stage-1 posrep.",
    scope: "out",
  },
  {
  ...
  {
    form: "- code is `age-code`. + - source representation: - type is Patient. - value element is Patient.birthDate. - value type is date. - value projection is age today <at least | at most | under | younger than> <N> years|months.",
    meaning:
...
const RULES: KitRule[] = [
  ...
  },
  {
    ...
    clauses: [
      {
        text: "The RECENCY (local `code is` + age posrep) concept is `type is Observation` (the recency merge emits an Observation-boolean retrieve). A STANDALONE age posrep's own `type is` is `Patient` (the projected carrier).",
        force: "invariant",
      ...
      {
        text: "AGE-ONLY guardrail: `value projection` is a bounded catalog whose ONLY sanctioned entry today is age today over `Patient.birthDate` — do NOT author a `value projection` over any other datum (rejected at the catalog boundary).",
        force: "invariant",
        test: "verifyLoop:patient-age-projection",
      },
    ],
