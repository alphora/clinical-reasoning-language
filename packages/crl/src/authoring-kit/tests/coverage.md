## 6.4.9 verify-loop invalid clause anchors removed

Baseline: bc05b043dc660b422e9e03b450b70506e8ad540c. Target: 88b0440dc1ddaa4ae536df61f5cb924fa82e8def. Kit schema 2.18; content hash 30e932e249f5c8f7b1d4f8d7b780b67fe21898339e1163c57e794553842f25a3. The two `verify-loop` RecordSet-preview clauses are `force: "default"`, but each carried a `test`. By `types.ts` a `test` is "a RESOLVABLE anchor the `invariant` clause names — REQUIRED when `force === \"invariant\"`", and `methodologyRequirements` is "the resolvable home an invariant clause's `test` anchors to". Their values were `verify-loop:record-status-{preview,boundaries}` — the RULE id, not the `verifyLoop:` methodology namespace — and no requirement with either id existed, so both anchors resolved to nothing. Measured across the whole kit: 37 clauses carry a `test`, and exactly those 2 are non-invariant; there is no other precedent. The force-coverage gate missed it because it only checked resolution when `force === "invariant"`.

- Kit updated: both `test` fields are DELETED. The first attempt at this fix instead ADDED two `methodologyRequirements` (`record-status-preview`, `record-status-boundaries`, applicability "All CRL authoring") so the anchors would resolve. Both review arms rejected that independently and they were right: `methodologyRequirements` are served to agents as retrieval context, so adding them would have turned optional `default` guidance into a durable per-policy obligation on every KE — a teaching change nobody reviewed, made under the heading of a correctness fix, with an applicability that is wrong on its face because a policy with no Condition/Observation RecordSet retrieve has nothing to check. Deleting loses nothing the 2.17 kit actually had. No clause text changed and no requirement was added, so no teaching moved; what changed is that two clauses stopped citing evidence that did not exist.
- Kit updated: `assertForceCoverage` in authoring-kit.test.ts is now bidirectional — an `invariant` clause must carry an anchor AND that anchor must resolve; a non-invariant clause must carry none. Checking resolution alone would still have permitted a `default` clause to carry a RESOLVING `verifyLoop:` anchor, which is the same category error wearing a valid name. Two negative rows were added to the malformed-metadata table using a REAL anchor (`verifyLoop:native-outcome-verification`) on `default` and on `validator-enforced`, because an unresolvable value would only re-prove the dangling-name check rather than the force/anchor pairing. Watched fail before trusting: restoring the original `verify-loop:` spelling throws `verify-loop: unresolved anchor verify-loop:record-status-preview`.
- Existing guidance sufficient: the owning assertions in `packages/crl/src/cre/tests/conditionStatus.test.ts` are unchanged, and so are their `@kit verify-loop:record-status-{preview,boundaries}` tags. Those tags are correct as written and are now the only link between the clauses and their evidence — which is what they were at 2.17. The `@kit` tag namespace is `<ruleId>:<claim>` (every tag in the suite uses a kebab-case rule id), a DIFFERENT namespace from a clause anchor; the two merely resemble each other, which is the confusion that produced the defect. No tag was added or removed and no claim lost an owning assertion.
- Kit updated (pins): the schema/content pins moved with the content, in all three places that hold them — `authoring-kit.test.ts`, `packages/crl/src/cli/tests/run-mcp-server.test.mjs` and `packages/crl-vscode/src/mcp-server.test.mjs`. The first pass missed the two MCP files and the full suite went red on them; a review arm predicted that before the run reported it.
- Not author-facing: root/core/extension version and lock metadata for 6.4.9; the FHIR-emitter PlanDefinition adaptive-marker change and its goldens (emit contract, no kit claim); the bleph acceptance `contract.json`/`sha256.json` repin and its README note; the `fix-pd-adaptive-marker` correction script and its tests; the crl-release and crl-cleanup skills; AGENTS.md, CLAUDE.md and mail-context.md. The previous audit stamp and this ledger are metadata.
- Unresolved gap: all 14 `it` declarations in conditionStatus.test.ts carry BOTH `record-status-preview` and `record-status-boundaries`, so neither claim has a distinguishing owning assertion. The assertions mostly split — the boundary cases are largely the refusal ones — but not cleanly: `conditionStatus.test.ts:133` (retains local and unprojected source records in one collection) and `:193` (union admission) are boundary claims that assert ADMISSION rather than refusal. Splitting the tags per claim is a separate kit pass; recorded here rather than silently corrected inside a release.

Changed paths for this correction: packages/crl/src/authoring-kit/index.ts; packages/crl/src/authoring-kit/tests/authoring-kit.test.ts; packages/crl/src/authoring-kit/tests/coverage.md; packages/crl/src/cli/tests/run-mcp-server.test.mjs; packages/crl-vscode/src/mcp-server.test.mjs. The following separate audit stamp changes audit.json and this ledger only. No compiler, CRE, CQL/FHIR emitter or engine implementation changed for this kit delta, and no new clinical or native-execution claim is made.

## 6.4.7 flat and grouped native question association

Baseline: 0a164fdc83b49d9ba9a9a9cb9261fb83f4399169. Target: bc05b043dc660b422e9e03b450b70506e8ad540c. Kit schema 2.17; content hash b94d87cf2fd940d71c56017b8e172e8d877f7fef80982808bdea84f64a7d9813. The engine adoption changed generated Questionnaire shape without changing the existing group-only teaching. A matching content hash did not establish that this runtime-dependent claim remained valid. This section corrects that earlier audit conclusion.

- Kit updated: concept-presentation now identifies the exact profile/value-path answer, verifies wording on the flat answer or its exact owning input group, and joins the response by both linkId and definition. No invented group or output repair. The unchanged presentation-check.cjs implements both shapes. Its tagged native-association controls cover grouped positive, missing/swapped text and wrong owner; flat positive, missing text, wrong response link and duplicate answers. The shared fixture is synthetic Observation/string, with no customer content. The already-produced 6.4.6 native Boolean Questionnaire/Response provides the separate bounded flat native observation. Neither tier establishes arbitrary hierarchies or native description rendering.
- Existing guidance sufficient: sibling-answer-preservation and emitted description checks remain unchanged. docs/shared-presentation-editing.md and intake.cjs receipt labels now use the same flat/grouped ownership wording. The reverse claim map includes the previously untagged flat test; one existing native-association tag was added, none removed, no claim loses its assertions.
- Not author-facing: root/core/extension version and lock metadata, kit schema/hash test pins, previous audit stamp and release-integration.md qualification receipts. The full baseline-to-release delta otherwise contains only those metadata/docs changes; no engine/evaluator/emitter/session implementation changed after the 6.4.6 engine audit. Historical ledger sections remain historical.

Changed paths for this correction: docs/shared-presentation-editing.md; package.json; package-lock.json; packages/crl/package.json; packages/crl-vscode/package.json; packages/crl/scripts/native-acceptance/intake.cjs; packages/crl/scripts/native-acceptance/presentation-check.test.cjs; packages/crl/src/authoring-kit/index.ts; packages/crl/src/authoring-kit/tests/coverage.md; packages/crl/src/authoring-kit/tests/reverse-claim-map.md; packages/crl/src/authoring-kit/tests/authoring-kit.test.ts; packages/crl/src/cli/tests/run-mcp-server.test.mjs; packages/crl-vscode/src/mcp-server.test.mjs. The following separate audit stamp changes audit.json and this ledger only. Installed delivery qualification is recorded separately.

## Local 6.4.6 engine-identity content correction

Baseline: ef474dc6885c11f04e1054e4cda31d2107ae4a64; target: 0a164fdc83b49d9ba9a9a9cb9261fb83f4399169. The export gate correctly rejected the earlier unchanged-content assumption below: the native-results rule interpolates ENGINE_JAR_SOURCE, so the engine build ID and cache path change the canonical kit payload. The inspected full diff from installed 6.4.5 contains only that rule's two engine identifiers plus version/hash/audit metadata. No semantic clauses or examples change. Kit schema advances to 2.16; content hash is d5099f1b7a0b82af0b23b2ec2706afec50dd23799da6ad43b12426a92598fbd4.
...
- Existing guidance sufficient: results/spawn.ts changes only default engine source/hash/cache/URL. verify-loop independent native proof and native-apply-session explicit Q/QR/state/error requirements remain correct. Exact unmodified upstream dcac972 adds asked-unknown metadata and deduplicates shared profiles. The generic shared-input reproduction now returns1question; three previous failing source-data controls return3distinctquestions with preserved outcomes. Compound false/unknown in both orders, first/nested pause and evaluated errors pass. Full six-step answer/change/clear and four-step dateTime/Coding controls pass with the unchanged installed driver. These are bounded native observations, not universal policy or clinical acceptance.
- Existing guidance sufficient: patches/cqframework/README.md, applicability-pause/README.md, cli-build.json and test/acceptance/bleph/README.md identify the unmodified upstream source and retire current-use instructions for local patches/configurable pause/QR-only extraction. Old patch files remain historical. Current session instructions require the returned Questionnaire by canonical; old Bleph QR-only evidence is explicitly unqualified for this engine. No diagnostic suppression added.
- Not author-facing: root/core/extension versions and lock; release-integration.md; audit metadata and this ledger. Later 6.4.5 MCP tests only repin the already reviewed2.15 identity; both real transports passed. New runtime delivery requires accessible exact engine bytes/default selection and installed6.4.6 Windows/Linux qualification; those are delivery gates, not a language change.
- Owning source tests: results/tests/spawn.test.ts acquisition/cache/hash checks and existing session/response/error tests. No assertions weakened. Prior kit source/native evidence is retained only where runtime checks support it. Review837 plan0findings;838 code1important accepted (obsolete QR-only recipe), final0findings. External unavailable.

Changed-path inventory since baseline: docs/release-integration.md; package.json; package-lock.json; packages/crl/package.json; packages/crl-vscode/package.json; packages/crl-vscode/src/mcp-server.test.mjs; packages/crl/src/cli/tests/run-mcp-server.test.mjs; packages/crl/src/authoring-kit/audit.json; packages/crl/src/authoring-kit/tests/coverage.md; packages/crl/src/authoring-kit/tests/maintenance-6.4.5-paths.txt; packages/crl/src/results/spawn.ts; packages/crl/test/acceptance/bleph/README.md; patches/cqframework/cli-build.json; patches/cqframework/README.md; patches/cqframework/applicability-pause/README.md.

## Local 6.4.5 record-status preview maintenance

Baseline: 72560b0b4b88cf93b875a395af22972d55e1535e (kit2.14). Target: 9adb36a190620817afb2241f409fac569733cf95. Focused kit, CRE and affected AST qualification: 713 tests across 30 files passed. Kit2.15 content 4be4afc2a6f18625eee21d08397a87330daf89d95c2d50df9faae09ec2a60278. Complete changed-path inventory: maintenance-6.4.5-paths.txt. This audit covers the complete baseline delta; installed delivery qualification is separate.

| Disposition | Changes and owning evidence | Scope and limits |
|---|---|---|
| Kit updated | verify-loop clauses and discovery aliases; conditionStatus.test.ts, "evaluates active and verified over the same CEL Condition that native execution receives" | Shared condition-status fixture and its package canonical base exercise Matching, Nonmatching and Empty through actual CEL emission, runCel and renderScenario. Explicit exists observes the authored set; it does not establish clinical completeness. Three independent retained-engine native controls agree with these activities. |
| Kit updated | conditionStatus.test.ts, status rows, same-record test, exact system/code and other-patient test, imported operand scope test | Five Condition rejection rows: missing clinical status, inactive, wrong clinical-status system, missing verification, refuted. Separate records cannot supply the two required statuses. These are evaluator controls over emitted resource objects, not five new native executions. |
| Kit updated | conditionStatus.test.ts, local/source union test, one/two-source emission comparison | Same-type unprojected local/source records are retained; mismatched resource kind refuses. One equal-value-type source representation emits and previews; two sources refuse in both lanes. No general collection union or projection support claimed. |
| Kit updated | conditionStatus.test.ts, cycles/unresolved references, locally coded narrative, ambiguous owners, unsupported operations and off-path tests | Reached failures stay errors. Owner rows cover source/local/scalar shadow; operation rows cover active during, most recent active and performed. Off-path unsupported collections do not contaminate reached supported paths. Limitations are implementation boundaries owned by the CRL lead, not language prohibitions or clinical negatives. |
| Kit updated | conditionStatus.test.ts, false-valued Observation existence and verification-status rows | Explicit exists differs from selected Boolean answer consumption. final/amended/corrected are retained; preliminary/missing are excluded. Boolean-annotated narrative filters refuse because emitter collection lowering is absent. These observations are CRE/evaluator evidence, not universal native parity. |
| Existing guidance sufficient | sourceMembership.ts terminology helper export; recordCollections.ts and run.ts implementation | Existing source-representation/library-scoping and independent native verification requirements remain. All 27 new test cases are accounted for above; both added tags map to verify-loop. No prior tag removed. Both real MCP transport suites passed after updating their schema/hash pins to kit2.15; extension MCP has36 checks. Existing 450 CRE cases pass. No new clinical example or customer fixture is introduced. |
| Existing guidance sufficient | concept-model-t1.test.ts, definedAsBooleanComposition-t1.test.ts and runInlineCel.ts | Seven presence/composition controls now resolve their unchanged CRL/CEL through a temporary project with explicit canonical base, registry and covers closure. All67 assertions still pass; no expected outcome removed or weakened. This supplies the same record emission prerequisites as production. |
| Existing guidance sufficient | emit-bundle-smoke.mjs changes Unix nonregular fixture from FIFO to /dev/null | fhir-packaging:definition-bundle-mcp still checks refusal before reading. EOF is bounded and works on the WSL mount; no product behavior changed. |
| Not author-facing | docs/release-integration.md, versions/lock, previous audit stamp and ledger finalization | Delivery/evidence metadata inspected; no new dependency, engine, public release or consumer-content migration. Prior metadata names the reviewed 6.4.4 source; source ancestry retains every local build. |
| Not author-facing | kit schema/hash pins, navigation additions, this ledger and path inventory, test tags | Retrieval and audit wiring for the bounded guidance; no duplicated behavior suite. Full canonical payload diff must contain only schema, two verify-loop clauses and their discovery metadata. |

Kit code review836 converged with zero findings; audit finalization and test-pin changes are mechanical. Source plan/code reviews833/834 converged with five important findings accepted. Local build plan835 accepts one important finding: run the installed npm preview on Linux as well as Windows; unchanged native/session evidence alone cannot qualify changed TypeScript. External review unavailable after the exhausted session retry. No Connectathon fixture or IEHP trial is part of this qualification. Broad existing engine/session proof is retained by exact asset identity; newly changed preview is qualified independently.

## Local 6.4.4 definition Bundle and native session maintenance

Baseline: 13a190955dddd169c84a9a768bd46a61f00592f4, kit2.13. Target: 72560b0b4b88cf93b875a395af22972d55e1535e; kit2.14 content ed2f2be40009500cbc7e01a2e6c6d51926e4101c96df243b021dd20659f76eaf. Source audit complete; installed qualification remains separate. Complete delta inventory: maintenance-6.4.4-paths.txt.

- Kit updated: fhir-packaging now distinguishes definitions-only emit_crl_bundle/emitCrlBundle from package_fhir. emit-bundle.test.ts owns exact real two-lane composition, embedded CQL, resource identity/source immutability, unmatched refs, physical filename collision, CQL import failure and missing-CQL refusal with no partial Bundle. Shared emit-bundle-smoke.mjs runs through both real MCP transports: no writes/patient/staticQ, bad paths/oversized input, authored syntax refusal; Unix non-regular /dev/null input refuses before read (WSL mount cannot create FIFOs). Tags fhir-packaging:definition-bundle and :definition-bundle-mcp added. No new clinical semantics or universal dependency-resolution claim.
- Kit updated: native-apply-session maps sessionResponse.test.ts typed edits (false, Coding, Quantity raw decimals, dateTime precision/offset), stale pointer, type/multiplicity/subject/canonical/version/hierarchy refusal, full versus edits-only, clear versus omission, repeated/answer-child constraints. session.test.ts owns immutable explicit input, correlation/runtime evidence, errors beside Questionnaire, bounded outputs, timeout/cancel/cleanup and failure artifacts. sessionErrors.test.ts covers actual embedded diagnostic shapes; sessionCli.test.ts covers preprocessing failure evidence, nonzero exit and preserving caller-owned directories. These are controlled unit/process boundaries, not native repeated-resource reconciliation. Tags :typed-edits and :failure-boundary added.
- Existing guidance sufficient: source-publication-identity.test.ts and changed foreignPublicationInterface/criterionEmit/membershipPublication assertions cover uncoded supporting sources beside a coded root, distinct physical CQL names, preserved publication canonicals, actual condition Library ownership and genuine code/slug collisions. Existing source-representation, library-scoping and identifier-only PlanDefinition guidance remains correct. New source tests do not define a new language form. Five native adult/minor/missing DOB and yes/no/missing answer controls pass on retained engine9870fc; no invented age question.
- Not author-facing: session CLI/SDK exports, esbuild and stableServer delivery, catalog copy, driver Java/class/source sidecars and runtime fingerprint env wiring implement the documented adapter. sessionBundle.test.mjs verifies physical runtime assets and API exports. Existing batch driver invocation remains; session mode is additive. Native helper and presentation association tests admit either grouped or direct question layouts with exact identity and independent sibling preservation. These broaden test oracles, not user-visible engine behavior. Prior audit/version/lock changes are metadata; no new dependency or engine adoption.
- Limits: latest upstream c34 emits a structurally invalid empty data-absent-reason extension on unknown condition results; adoption is held. Six candidate steps pass functionally but do not establish conformance or mixed-condition completeness. Retained-engine six-step cumulative text/Boolean answer/change/clear checks pass; installed Windows/Linux qualification is recorded separately in the6.4.4 delivery receipt; no shipped/native/client-persistence claim here.

Reviews825/826 source fix and828/829 Bundle converge natively; external coverage unavailable. Session code reviews810/818 already converged; cumulative integration preserves all6.4.3 changes. Five Bundle unit tests and actual core/extension Bundle MCP checks pass; complete kit/source and installed qualification follow. No prior kit tags removed; untagged helper/type/packaging changes are dispositioned above.
...
Owning verification: routeCards.test.mjs adds three-owner/three-input grouping with real inherited presentation resolution, separate libraries, preserved Unknown, paired criterion paths and numbering. Existing distinct scoped wording assertion now also requires one semantic question number. routeQuestionDedup.browser.cjs verifies3questionnaire/9tree placements, edits from a secondary owner, every selected-choice marker, distinct variant proposal IDs, delayed success/failure state, value-only+Unknown visibility and stale response rejection. These are UI assertions, not clinical or native semantics claims. Native reviews823/824 accepted all5important findings and converged; external arm unavailable after exhausted attempts.24focused model/panel tests, extension compile/typecheck, new browser and existing browser regression checks pass. Actual installed6.4.3 extension renders3editable inputs for a synthetic3condition route; installed npm emitted that fixture through the retained engine. Both installed MCPs return contentMatchesAudit:true. Prior6.4.2 full-suite/native/packaging evidence retained; no full retest.

# Maintenance - coded Observation source local 6.4.2

Baseline audit: `5020abfe4c509c20ecf65e11dcaba401193b5d29`. Implementation base: `5574fc1d349e06f52b593ddb14e2cbc4e36878f3`. Target: `a9c841c416078343b369752cc99ca66e53292e44`. Kit schema2.13/hash `daf1df90d2aa8796f3cbc1de923e1c8e0f4f3d257c3dcd5c492a6ee2ba9caa4c`. Reviewed source audit complete; installed qualification follows.
