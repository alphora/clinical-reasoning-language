...
- **Two Inferred defines** in one library:
  ...
  - `define "Top": "example-nested-Inferred"."A And B" union "example-nested-LocalSource"."Diagnosis C"`
  (the nested define references the sibling Inferred define for `A And B`, and
  the LocalSource define for the `code is` leaf `Diagnosis C`).
