 ...
 *   emit-activities-suppress-target-unresolved     error    A suppressed activities-only library (its empty CQL/Library dropped by the CQL lane, #186) names a consumer decision library that emitted NEITHER an Interface NOR a name-keeping Root, so the rebound ActivityDefinition/Recommendation `library[]` would dangle. Belt-and-suspenders: unreachable on the integrated `emitFhirDefFromPath` path (same `emitCQLImports` result feeds both manifest and bindings); guards the exported `emitFhirDefClosure` against a hand-built mismatched (manifest, bindings) pair. (#186)
 *   (emit-casefeature-non-boolean was REMOVED — the LocalPrimitives-always-boolean rule: every `code is` concept is a boolean Observation case-feature regardless of declared value type, so there is no value-type gate.)
