...
export async function runtimeFingerprint(javaExe: string, engineJarPath: string, bounds: JvmBounds, signal?: AbortSignal):
Promise<{ sha256?: string; cleanupConfirmed: boolean; reason?: string }> {
  ...
  const probe = await runOwnedProcess(javaExe, args, { timeoutMs: 30000, maxBytes: 65536, signal });
