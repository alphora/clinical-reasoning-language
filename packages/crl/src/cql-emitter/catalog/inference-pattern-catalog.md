...
>
> **v0.7.0 — catalog migration paired with `definition is` keyword.** The 6 `<X> is <state>` patterns (Active, IsVerified, High, Low, Normal, Abnormal) drop their inner `is`, becoming `<X> active`, `<X> verified`, `<X> high/low/normal/abnormal`. The change pairs with CRL v0.7's renamed `definition is` body keyword (see `.vibe-tools/discussions/017-logic-is-keyword-rename.md` round 4 Path 1): under the new keyword, `definition is "X" is active` reads as doubly-copular, so the catalog migration drops the inner `is` to eliminate the collision. Canonical names (`Active`, `IsVerified`, `High`, `Low`, `Normal`, `Abnormal`) and CQL function names (`CRLCommon.Active`, etc.) unchanged — only the narrative-surface form changes. Same surgery as the v0.5.5 `WasOrdered`/`WasPerformed` migration that dropped `was`.
>
> **v0.7.0 (companion) — CRLCommon library refactored to v0.2.0** (generics-by-composition; emitter v0.3). Catalog narrative + canonical + CQL function names ARE unchanged. The library implementation moved to primitive list-returning forms (filter patterns return `List<Resource>` instead of `Boolean`), so each pattern has ONE natural output shape. The author's `(type, valuetype)` declaration selects the pattern's REALIZATION FORM — a catalog signature never constrains what may be declared. ⚠ The emitter must NEVER insert a reduction (`exists`, a singleton lift, a `Coalesce`) to bridge a declared shape; a mismatch is an author-time error naming the reduction to declare. (The emitter still bridges today — that code is the patient, pending reduction nesting.) See [[patterns-are-semantic]] for the principle; `src/cql-emitter/catalog/CRLCommon.cql` v0.2.0 header for the implementation rationale.
>
> **v0.6.0 — "What not How" sweep.** Canonical signatures no longer carry return-type annotations (the `: boolean` / `: Period` / `: Instance<X>` / `: WindowSpec` / `: T` parts after the closing paren). Per the v0.6 design discovered while correcting cms69/cms22: `defined as` is **semantic inference** (normalizing one concept's representations/components into one fact — NOT decision composition, #168), not boolean logic — the CRL author declares each concept's `(type, valuetype)` based on semantic intent; the catalog describes pattern FORM (narrative + canonical name + parameter types for autocomplete + CQL function name), not return SHAPE. Treating catalog return types as authoritative for concept shape caused a multi-round correction loop (see `.vibe-tools/discussions/016`). Return types still appear in the CQL function definitions shown later in the document (those are CQL-side / emitter-side, accurate description of the function's CQL return type). The canonical column of the reference table is now author-facing form only.
>
...
CRLCommon.Justified("Primary-Care Referrals" expression, "Hypertensive Reading Findings" expression)
  and CRLCommon.WasOrdered("Primary-Care Referrals" expression)
```
...
**Validator and emitter implications.** The validator template-matches narrative against the catalog and type-checks the canonical AST against the `canonical` signature. The emitter consumes canonical AST + the `CQL function` field, emitting calls into the shared `CRLCommon.cql` library.
...
The emitter knows the concept's kind from its declaration and inserts the correct CQL expression at the call site. Pattern functions in `CRLCommon.cql` therefore take resolved expressions (lists, intervals, resources, values) — not strings.
...
The CRLCommon library handles `CRLCommon.AsOf` and `CRLCommon.AgeAt` as **CQL-level overload sets**, one overload per resolved-anchor type. `AgeAt` follows the same overload-set pattern as `AsOf`. Concretely (AsOf shown; AgeAt is parallel):
...
|---|---|---|---|
| `Has(X)` | `has <X>` | `Has(X: ConceptRef)` | `CRLCommon.Has` |
| `HasHistoryOf(X[, anchor])` | `has history of <X>` (optionally `prior to <anchor>`) | `HasHistoryOf(X: ConceptRef[, anchor: ConceptRef])` | `CRLCommon.HasHistoryOf` |
| `Without(kind, X)` | `without <kind> <X>` (kind ∈ record-of, documented, evidence-of, result-for, …) | `Without(kind: KindEnum \| ConceptRef, X: ConceptRef \| Disjunction<ConceptRef>)` | `CRLCommon.Without` |
| `CurrentlyTaking(med)` | `currently taking <med>` | `CurrentlyTaking(med: ConceptRef)` | `CRLCommon.CurrentlyTaking` |
| `HasAdverseReactionTo(X)` | `has adverse reaction to <X>` | `HasAdverseReactionTo(X: ConceptRef)` | `CRLCommon.HasAdverseReactionTo` |
...
<!-- | `With(X, Y)` | `<X> with <Y>` | `With(X: ConceptRef, Y: ConceptRef \| SubjectBoundPredicate)` | `CRLCommon.With` | -->
...
<!-- | `Within(X, window)` | `<X> within <window>` — window is a named period OR a window-from-anchor | `Within(X: ConceptRef, window: ConceptRef \| WindowSpec)` | `CRLCommon.Within` | -->
| `ComponentOf(panel, discriminator)` | `<discriminator> component of <panel>` | `ComponentOf(panel: ConceptRef, discriminator: ConceptRef<T>)` (T from discriminator's valuetype) | `CRLCommon.ComponentOf` |
| `NotDoneWithReason(action, reason)` | `<action> not done with reason <reason>` (reason may be a disjunction `(<A> or <B>)`) | `NotDoneWithReason(action: ConceptRef, reason: ConceptRef \| Disjunction<ConceptRef>)` | `CRLCommon.NotDoneWithReason` |
<!-- #99 deferred: shape collision with the broader `<X> with <Y>` (With) form. -->
<!-- | `BaselineAndFollowUp(initial, followup)` | `<initial> with follow-up <followup>` | `BaselineAndFollowUp(initial: ConceptRef, followup: ConceptRef)` | `CRLCommon.BaselineAndFollowUp` | -->
<!-- #99 deferred: 5+ element narrative not wired in matcher; needs the `inpatient stay anchored on <X> [including prelude]` form added. -->
<!-- | `InpatientStay(encounter[, includePrelude])` | `inpatient stay anchored on <encounter>` (optionally `including prelude`) | `InpatientStay(encounter: ConceptRef[, includePrelude: boolean = false])` | `CRLCommon.InpatientStay` | -->
| `WasOrdered(X)` | `<X> ordered` | `WasOrdered(X: ConceptRef)` | `CRLCommon.WasOrdered` |

### Assertion

| Pattern | Narrative form | Canonical | CQL function |
|---|---|---|---|
| `Justified(action, reason)` | `<action> justified by <reason>` | `Justified(action: ConceptRef, reason: ReasonExpr)` (see Type notation — heterogeneous disjunction supported) | `CRLCommon.Justified` |
| `Active(X[, during])` | `<X> active` (optionally `during <period>`) | `Active(X: ConceptRef[, during: ConceptRef])` | `CRLCommon.Active` |
| `IsVerified(X)` | `<X> verified` | `IsVerified(X: ConceptRef)` | `CRLCommon.IsVerified` |
| `DocumentedAs(X, classification)` | `<X> documented as <classification>` | `DocumentedAs(X: ConceptRef, classification: ConceptRef)` | `CRLCommon.DocumentedAs` |
...
| Pattern | Narrative form | Canonical | CQL function |
|---|---|---|---|
| `MostRecent(X[, scope])` | `most recent <X>` (optionally `<scope>`) | `MostRecent(X: ConceptRef[, scope: ScopeSpec])` | `CRLCommon.MostRecent` |
| `Last(X[, scope])` | `last <X>` (optionally `<scope>`) | `Last(X: ConceptRef[, scope: ScopeSpec])` | `CRLCommon.LastOf` |
| `Earliest(X[, scope])` | `earliest <X>` (optionally `<scope>`) | `Earliest(X: ConceptRef[, scope: ScopeSpec])` | `CRLCommon.Earliest` |
| `First(X[, scope])` | `first <X>` (optionally `<scope>`) | `First(X: ConceptRef[, scope: ScopeSpec])` | `CRLCommon.FirstOf` |
| `During(event, period)` | `<event> during <period>` | `During(event: ConceptRef, period: ConceptRef)` | `CRLCommon.During` |
| `Overlaps(eventA, eventB)` | `<eventA> overlaps <eventB>` | `Overlaps(eventA: ConceptRef, eventB: ConceptRef)` | `CRLCommon.Overlaps` |
| `OnDayOfOrAfter(X, anchor)` | `<X> on day of or after <anchor>` | `OnDayOfOrAfter(X: ConceptRef, anchor: ConceptRef)` | `CRLCommon.OnDayOfOrAfter` |
| `OnOrBefore(X, anchor)` | `<X> on or before <anchor>` | `OnOrBefore(X: ConceptRef, anchor: ConceptRef)` | `CRLCommon.OnOrBefore` |
| `SameDay(eventA, eventB)` | `<eventA> same day as <eventB>` | `SameDay(eventA: ConceptRef, eventB: ConceptRef)` | `CRLCommon.SameDay` |
...
<!-- | `BetweenAnchors(X, start, end)` | `<X> between <start> and <end>` (start/end are concept refs — see dispatch rule) | `BetweenAnchors(X: ConceptRef, start: ConceptRef, end: ConceptRef)` | `CRLCommon.BetweenAnchors` | -->
| `AtLeastApart(eventA, eventB, duration)` | `<eventA> and <eventB> at least <duration> apart` | `AtLeastApart(eventA: ConceptRef, eventB: ConceptRef, duration: Quantity<time>)` | `CRLCommon.AtLeastApart` |
| `AtMostApart(eventA, eventB, duration)` | `<eventA> and <eventB> at most <duration> apart` | `AtMostApart(eventA: ConceptRef, eventB: ConceptRef, duration: Quantity<time>)` | `CRLCommon.AtMostApart` |
...
| Pattern | Narrative form | Canonical | CQL function |
|---|---|---|---|
| `before-start-of(duration, anchor)` | `<duration> before start of <anchor>` | `BeforeStartOf(duration: Quantity<time>, anchor: ConceptRef)` | `CRLCommon.BeforeStartOf` |
| `after-start-of(duration, anchor)` | `<duration> after start of <anchor>` | `AfterStartOf(duration: Quantity<time>, anchor: ConceptRef)` | `CRLCommon.AfterStartOf` |
| `before-end-of(duration, anchor)` | `<duration> before end of <anchor>` | `BeforeEndOf(duration: Quantity<time>, anchor: ConceptRef)` | `CRLCommon.BeforeEndOf` |
| `after-end-of(duration, anchor)` | `<duration> after end of <anchor>` | `AfterEndOf(duration: Quantity<time>, anchor: ConceptRef)` | `CRLCommon.AfterEndOf` |
| `on-day-of(anchor)` | `on day of <anchor>` | `OnDayOf(anchor: ConceptRef)` | `CRLCommon.OnDayOf` |
...
<!-- | `AtLeastN(events, n)` | `at least <n> <events>` | `AtLeastN(events: ConceptRef, n: Integer)` | `CRLCommon.AtLeastN` | -->
<!-- #83 grammar gap: leading integer parsed as start of a value-threshold quantity; `consecutive` is unexpected. -->
<!-- | `Consecutive(events, n)` | `<n> consecutive <events>` | `Consecutive(events: ConceptRef, n: Integer)` | `CRLCommon.Consecutive` | -->
| `High(X)` | `<X> high` | `High(X: ConceptRef<Quantity>)` | `CRLCommon.High` |
| `Low(X)` | `<X> low` | `Low(X: ConceptRef<Quantity>)` | `CRLCommon.Low` |
| `Normal(X)` | `<X> normal` | `Normal(X: ConceptRef<Quantity>)` | `CRLCommon.Normal` |
| `Abnormal(X)` | `<X> abnormal` | `Abnormal(X: ConceptRef<Quantity>)` | `CRLCommon.Abnormal` |
| `AtLeast(value, target)` | `<value> at least <target>` | `AtLeast(value: ConceptRef<Quantity<U>>, target: Quantity<U>)` | `CRLCommon.AtLeast` |
| `AtMost(value, target)` | `<value> at most <target>` | `AtMost(value: ConceptRef<Quantity<U>>, target: Quantity<U>)` | `CRLCommon.AtMost` |
| `Between(value, lo, hi)` | `<value> between <lo> and <hi>` (lo/hi are Quantity literals — see dispatch rule) | `Between(value: ConceptRef<Quantity<U>>, lo: Quantity<U>, hi: Quantity<U>)` | `CRLCommon.Between` |
| `Exceeds(value, target)` | `<value> exceeds <target>` | `Exceeds(value: ConceptRef<Quantity<U>>, target: Quantity<U>)` | `CRLCommon.Exceeds` |
| `Below(value, target)` | `<value> below <target>` | `Below(value: ConceptRef<Quantity<U>>, target: Quantity<U>)` | `CRLCommon.Below` |

### State / Process Inference

| Pattern | Narrative form | Canonical | CQL function |
|---|---|---|---|
| `WasPerformed(X)` | `<X> performed` | `WasPerformed(X: ConceptRef)` | `CRLCommon.WasPerformed` |
...
- Emitter: `CQL function` column maps each pattern to its `CRLCommon.<Name>` target; arguments flow through the parameter-emission-mode table.
