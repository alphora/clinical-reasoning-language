...
>
> **v0.7.0 (companion) — CRLCommon library refactored to v0.2.0** (generics-by-composition; emitter v0.3). Catalog narrative + canonical + CQL function names ARE unchanged. The library implementation moved to primitive list-returning forms (filter patterns return `List<Resource>` instead of `Boolean`); the emitter wraps with `exists(...)` at the call site when the author declares a boolean consumer. Author's `(type, valuetype)` declaration is now authoritative end-to-end. See [[defined-as-is-semantic-composition]] / [[patterns-are-semantic]] for the principle; `src/cql-emitter/catalog/CRLCommon.cql` v0.2.0 header for the implementation rationale.
>
