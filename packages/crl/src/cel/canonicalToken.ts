...
/** Resolve a bare coded answer against the complete declared answer domain. Qualified tokens remain explicit. */
export function parseCodedValueToken(
  raw: string,
  members?: readonly { system: string; code: string }[],
): { parts: CodeParts } | { error: string } {
  if (raw.includes("|") || members === undefined) return parseCheckedCanonicalToken(raw);
  const code = raw.trim();
  const matches = members.filter((member) => member.code === code);
  if (matches.length !== 1) return { error: matches.length === 0
    ? `Code '${code}' is not offered by the concept's named answer ValueSet. Use a declared code or an explicit <system>|<code> token.`
    : `Code '${code}' belongs to multiple systems in the answer ValueSet. Use an explicit <system>|<code> token.` };
  return { parts: { system: matches[0].system, code } };
}
