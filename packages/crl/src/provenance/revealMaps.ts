...
//
// Lives in `crl/provenance` (#170) so `validate_provenance` can gate cockpit correspondence against this REAL
// resolution (not a reimplementation). It depends only on `crl` types + `buildConceptContainment`; `crl-vscode`
// re-exports it for the renderer (correspondenceCockpit). Do NOT add a `crl-vscode` dependency here.
import { buildConceptContainment } from "./conceptContainment";
import type { CorrespondenceModel } from "./correspondence";
import type { CrlConceptNode } from "./crlConceptLayer";
import type { CrlDecisionStructure, CrlStructureNode } from "./crlStructure";
