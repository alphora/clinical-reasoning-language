...
describe("deriveCoverage — sense 1 (Missed₁) §4", () => {
    ...
    const clusters: Cluster[] = [
      {
        ...
        label: "x",
        items: ["n1"],
        crl: [crlRef("Crit", "concept", "implements-criterion", "linked")],
        cel: [],
      },
    ];
    ...
    const prov: Cluster[] = [
      {
        ...
        label: "x",
        items: ["n1"],
        crl: [crlRef("Crit", "concept", "implements-criterion", "provisional")],
        cel: [],
      },
    ];
    expect(
      deriveCoverage(artifact({ items, clusters: prov }), idx, "crit").missedDecisions,
    ).toEqual(["n1"]);
  });
  ...
  it("wrong relation / SHARED node / cross-item do not satisfy decision-implementation", () => {
    ...
    const wrongRel: Cluster[] = [
      {
        id: "c1",
        label: "x",
        items: ["n1"],
        crl: [crlRef("Crit", "concept", "defines-concept", "linked")],
        cel: [],
      },
    ];
    expect(
      deriveCoverage(artifact({ items, clusters: wrongRel }), idx, "crit").missedDecisions,
    ).toEqual(["n1"]);
    // a linked recommends-disposition ref to a SHARED-lib activity → ownership shared, not policy-owned → not implemented
    const sharedRef: CrlNodeRef = {
      lib: "Shared Lib",
      kind: "activity",
      name: "SharedAct",
    ...
    const sharedCluster: Cluster[] = [
      { id: "c1", label: "x", items: ["n1"], crl: [sharedRef], cel: [] },
    ];
    expect(
      deriveCoverage(artifact({ items, clusters: sharedCluster }), idx, "crit").missedDecisions,
    ).toEqual(["n1"]);
    const crossItem: Cluster[] = [
      {
        id: "c1",
        label: "x",
        items: ["other"],
    ...
    expect(
      deriveCoverage(artifact({ items, clusters: crossItem }), idx, "crit").missedDecisions,
    ).toEqual(["n1"]);
  });
  ...
  it("a linked decision-relation ref not resolvable in the index → unresolvedLinkedRefs (not a silent Missed₁ only)", () => {
    ...
    const clusters: Cluster[] = [
      {
        ...
        label: "x",
        items: ["n1"],
        crl: [crlRef("Ghost", "concept", "implements-criterion", "linked")],
        cel: [],
      },
    ];
    ...
    expect(
      deriveCoverage(artifact({ clusters: authoredOnly }), idx, "").unresolvedLinkedRefs.map(
        (u) => u.cluster,
      ),
    ).toContain("cX");
  });
...
describe("deriveCoverage — sense 2 (Missed₂) §4", () => {
  ...
  it("a non-whitespace uncovered span is Missed₂; covered by a sourceRef OR ignoredRange it is not; whitespace gaps drop", () => {
    const covered: Item[] = [
      { id: "n1", origin: "source", text: "Adults qualify.", sourceRefs: [{ start: 0, end: 15 }] },
    ];
    ...
    const r2 = deriveCoverage(
      artifact({ items: covered, ignoredRanges: [{ start: 15, end: 27, reason: "noise" }] }),
      idx,
      text,
    );
  ...
  it("out-of-range / off-boundary offsets are surfaced (malformedRanges), not thrown — sourceRef AND ignoredRange", () => {
    ...
    const r2 = deriveCoverage(
      artifact({
        items: [{ id: "n2", origin: "source", text: "x", sourceRefs: [{ start: 0, end: 4 }] }],
      }),
      idx,
      "café",
    );
    ...
    const r3 = deriveCoverage(
      artifact({ ignoredRanges: [{ start: 0, end: 9999, reason: "x" }] }),
      idx,
      "café",
    );
  ...
  it("an uncovered span with a MULTIBYTE char renders correctly (byte arithmetic, not UTF-16) and never throws", () => {
    ...
    const items: Item[] = [
      { id: "n1", origin: "source", text: "café", sourceRefs: [{ start: 0, end: 5 }] },
    ];
...
describe("deriveCoverage — over-reach §4", () => {
  const names = (r: { overReach: { ref: { name: string; nodeId?: string } }[] }) =>
  ...
  it("a linked cluster ref removes a node from over-reach; an authored `supports` suppresses; intentionally-unlinked suppresses", () => {
    const linked: Cluster[] = [
      {
        ...
        label: "x",
        items: [],
        crl: [crlRef("Approve", "activity", "recommends-disposition", "linked")],
        cel: [],
      },
    ];
    ...
    const suppressed: Cluster[] = [
      {
        id: "c1",
        label: "x",
        items: ["auth"],
        crl: [crlRef("Approve", "activity", "recommends-disposition", "provisional")],
        cel: [],
      },
    ];
    expect(names(deriveCoverage(artifact({ items, clusters: suppressed }), idx, ""))).not.toContain(
      "Approve",
    );
    ...
    const intl: Cluster[] = [
      {
        ...
        label: "x",
        items: [],
        crl: [crlRef("Approve", "activity", "recommends-disposition", "intentionally-unlinked")],
        cel: [],
      },
    ];
