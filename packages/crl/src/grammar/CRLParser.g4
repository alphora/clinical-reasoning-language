...
// Presentation is an authored annotation, separate from computational declarations.
presentationStatement
    : PRESENTATION_FOR conceptReference COLON presentationLine+
    ;
presentationLine
    : DASH (PRESENTATION_DECISION decisionReference
           | PRESENTATION_CRITERION qualifiableReference
           | QUESTION_TEXT_IS (QUOTED_STRING | backtickString)
           | QUESTION_DESCRIPTION_IS (QUOTED_STRING | backtickString)) DOT
    ;
...
valueFromLine
    // REFACTOR:grounded (#320, 615): all recognized members qualify except explicit exclusions.
    : DASH VALUE_FROM terminologyReference (DOT | COLON notQualifyingLine+)
    ;

notQualifyingLine
    : DASH NOT_QUALIFYING_IS backtickString DOT
    ;
