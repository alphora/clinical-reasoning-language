...
This release adds the Medical Validation Question–Result view, route question cards,
a separate Result Questionnaire, and MV-scoped wording proposals for the owning KE.
CRL/CEL syntax and CQL/FHIR emission semantics are unchanged from 5.2.0. Core APIs
add selected answer values, presentation field ownership, and execution-model access
without source correspondence. Policy storage can be discovered from `src/crl` plus
`src/cel` without provenance; source highlighting still requires correspondence.
Authoring kit 2.2 documents MV case intent and the wording-patch handoff. Folder-based
MV/regression emission filtering is not implemented. See the [release notes](https://github.com/alphora/clinical-reasoning-language/releases/tag/v5.3.0)
and [MV guide](https://github.com/alphora/clinical-reasoning-language/blob/v5.3.0/docs/mv-component-view.md).

## Overview

See the [User Guide](https://github.com/alphora/clinical-reasoning-language/blob/v5.3.0/packages/crl/USER_GUIDE.md) for a comprehensive introduction to the CRL language, syntax, and authoring best practices. For the **CLI and MCP tool reference** (emit, validate, the MCP tools — for both CRL and CEL), see [TOOLING.md](https://github.com/alphora/clinical-reasoning-language/blob/v5.3.0/packages/crl/TOOLING.md).
...
read or save it. JSON remains the default. See the [kit guide](https://github.com/alphora/clinical-reasoning-language/blob/v5.3.0/docs/authoring-kit.md).

## Metadata annotations

CRL concepts can carry typed metadata via an `@tag` convention on `meta` lines (e.g. `@description`, `@ke-feedback`, `@kg-concept`) — for descriptions, knowledge-engineer feedback, plain-language logic, external-store hints, and extraction provenance. See the [metadata model](https://github.com/alphora/clinical-reasoning-language/blob/v5.3.0/packages/crl/spec/metadata-model.md) and the canonical [tag registry](https://github.com/alphora/clinical-reasoning-language/blob/v5.3.0/packages/crl/spec/metadata-registry.json). *(Draft: the convention parses today; Validator enforcement is forthcoming.)*
...
```bash
npm install @smile-digital-health/crl@5.3.0
```

Pin the version so generated-artifact migrations are adopted deliberately:

```json
"dependencies": {
  "@smile-digital-health/crl": "5.3.0"
}
```

CRL 5.2 renames generated retrieval libraries from `LocalPrimitives`/`ExternalPrimitives` to `LocalElements`/`ExternalElements`, including CQL includes and FHIR Library identities. Regenerate CQL and FHIR together and replace the previous generated-file inventory, preserving independently authored files. Re-emission does not automatically delete obsolete artifacts. See the [5.2.0 migration notes](https://github.com/alphora/clinical-reasoning-language/releases/tag/v5.2.0). A `^5` or `^5.1.0` dependency can acquire this update automatically; use an exact version pin when upgrades require coordinated artifact replacement.

CRL/CEL syntax is unchanged by 5.2. When upgrading from 5.0.0, also update the authoring-kit npm/MCP calls described in the [5.1.0 release notes](https://github.com/alphora/clinical-reasoning-language/releases/tag/v5.1.0). Content from before the CRL5 language transition may need language migration using the current authoring kit.
...
For more details, see the [User Guide](https://github.com/alphora/clinical-reasoning-language/blob/v5.3.0/packages/crl/USER_GUIDE.md) and the technical mapping documentation.
...
For semantic rules, layout details, deliberate spec deviations, and the MCP `emit_crl` tool, see [`USER_GUIDE.md` §"Emitting FHIR Definition resources"](https://github.com/alphora/clinical-reasoning-language/blob/v5.3.0/packages/crl/USER_GUIDE.md#emitting-fhir-definition-resources).
...
`validateCRL` performs single-file semantic validation. Inspect `success`, `errors` and `warnings`; use the path-based CLI or MCP validator when imports need project context.
...
> 
> **[.github/PULL_REQUEST_TEMPLATE/release.md](https://github.com/alphora/clinical-reasoning-language/blob/v5.3.0/.github/PULL_REQUEST_TEMPLATE/release.md)**
>
...
For more details, see the [release PR template](https://github.com/alphora/clinical-reasoning-language/blob/v5.3.0/.github/PULL_REQUEST_TEMPLATE/release.md) and the automation scripts in `.github/scripts/`.
...
For installation instructions and full details, see [crl-vscode/README.md](https://github.com/alphora/clinical-reasoning-language/blob/v5.3.0/packages/crl-vscode/README.md)
