...
Verification on2026-09-07:116/116 native passes and116/116 independent matching CRE predictions, including49 pauses,37 Met and30 Unmet. The public command built the core and ran the original pinned engine with explicit JVM encoding/locale settings. Native review then corrected two true-OR controls whose false conformance had masked the unknown operand. Both corrected cases passed fresh native executions. Final acceptance uses114 unchanged rows from `tmp/577-native` and2 corrected rows from `tmp/578-corrected-native`; `acceptance.json` in the latter directory records each evidence root, final input hashes, complete verdicts and808 unchanged dist-file hashes. The original run manifest retains its actual startup hashes. All33 checker/lifecycle controls pass. The original97 inputs remain byte-for-byte unchanged. Local plan/code reviews and their dispositions are in discussions577/578: the native finding was corrected and verified; the external reviewer returned internal errors and supplied no coverage.
