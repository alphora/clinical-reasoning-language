...
> **Status: core stable (v0.3.0); enforcement + status-aware emit LIVE (core).** The core vocabulary is settled (two design-review rounds); **v0.2.0 adds the review-flag tags** per KE #203; **v0.3.0 (Todo 5)** makes CQL emit **status-aware** (open flags emit; resolved suppress). **Enforcement (#154 / KE #203 Todo 3):** `validateCRL` enforces the core registry rules — vocabulary (malformed/unknown `@tag` → warning), field shape (required/enum/duplicate), cardinality, and the open-flag warning — via `MetaTagValidator` reading the compile-time-inlined registry; the `@tag` carrier parses on `concept`, `decision`, and `library` (Todo 2). **Emit (Todo 5):** a tag emits to a CQL block comment iff `emit.cql:true` AND its status is not in `emit.suppressWhenStatus` — concept scope only (`emit.cqlScopes:["concept"]`; decision→FHIR `.meta` is #206, library flags are gate-only). **Remainder (#154 follow-up):** the family-C extraction-exhaust rules (`run`-required, re-run staleness) + re-add detection. Canonical registry: [`metadata-registry.json`](./metadata-registry.json).
...
- **Emit (LIVE, Todo 5):** an `open` flag emits to a CQL block comment — normal meta-tag behavior (a KE sees open work in the generated CQL); a `resolved` flag does **not** emit (noise). Status-aware emit is enforced in `renderMetaBlock` (emit iff `emit.cql:true` ∧ `status ∉ emit.suppressWhenStatus`; only an EXACT `resolved` suppresses — the emitter runs no validator, so an annotated/unknown status conservatively emits). Emit is **orthogonal to `mvComplete`**: the gate reads the `.crl` flag status (`collectFlags`), never the emitted CQL. Emit is **concept-scoped** (`emit.cqlScopes:["concept"]`); a decision flag's emit home is the FHIR PlanDefinition `.meta` (#206), and a **library** flag is **gate-only** (no `crl library` wrapper is emitted, so nowhere to render — it still blocks `mvComplete`).
...
- **Detail link (`; ref`, KE #203 Todo 6):** a review flag may carry an OPTIONAL `; ref <issue>` pointing to the rich-detail tracker issue — the KE files the issue at flag CREATION; the `.crl` flag stays LEAN (gist + fields). Registry-modeled on all four flag tags (v0.3.1, `required:false`). Distinct from `@gap-filed`, whose `; ref` is REQUIRED. (Just the link — the tags→issues→PR loop is #204.)
...
- **Scope:** flags are authored at `concept`, `decision`, or `library` scope (Todo 2 carrier). The `mvComplete` gate is cross-scope (`collectFlags` reads all three). CQL **emit** is concept-only (see the Emit bullet): decision-flag emit → FHIR `.meta` (#206), library flags are gate-only.
