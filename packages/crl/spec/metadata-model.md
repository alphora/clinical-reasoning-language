...
## Review flags → the `.crl/flags/` store (relocated in #212)

Review flags were once carried here as `.crl` meta tags with `flag: true`. As of **#212 they left the `.crl` model entirely** and live as first-class **store records** under `<policy>/.crl/flags/<id>.json`:
...
- **Record model + store:** [`flags/mvFlag.ts`](../src/flags/mvFlag.ts) + [`flags/mvFlagStore.ts`](../src/flags/mvFlagStore.ts) (a self-describing `MvFlag` record with an anchor; per-flag JSON under `.crl/flags/`).
...
- **Review flags relocated to a `.crl/flags/` store (#212)** — originally they reused `@ke-feedback`'s status machinery as `.crl` meta tags (open→resolved, resolved-durable); as the model matured they became first-class store records (a self-describing `MvFlag`), decoupling the audit trail from `.crl` source. See the relocation section above.
- **`@description` is a meta tag**, not a first-class element (uniform; would otherwise need a grammar change).
- **Meta tags are inline; `.crl` is their source of truth** — no sidecar for meta; staleness handled by the re-run replace rule. (Review flags are the exception — a `.crl/flags/` store, #212.)
