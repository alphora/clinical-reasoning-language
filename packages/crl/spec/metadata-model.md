...
> **Status: core stable (v0.2.0); enforcement LIVE (core).** The core vocabulary is settled (two design-review rounds); **v0.2.0 adds the review-flag tags** per KE #203. **Enforcement (#154 / KE #203 Todo 3):** `validateCRL` now enforces the core registry rules — vocabulary (malformed/unknown `@tag` → warning), field shape (required/enum/duplicate), cardinality, and the open-flag warning — via `MetaTagValidator` reading the compile-time-inlined registry; the `@tag` carrier parses on `concept`, `decision`, and `library`. **Remainder (#154 follow-up):** the family-C extraction-exhaust rules (`run`-required, re-run staleness) + re-add detection. Canonical registry: [`metadata-registry.json`](./metadata-registry.json).
