      ...
      if (this.isWhitespace(c)) {
        if (c === '\n'.charCodeAt(0)) {
          ...
          if (this._currentIndent > previousIndent) {
            ...
            while (
              this._indentStack.length > 1 &&
              this._indentStack[this._indentStack.length - 1] > this._currentIndent
            ) {
              this._indentStack.pop();
              this._pendingTokens.push(this.createToken(TokenTypes.DEDENT, ''));
            }
          ...
          if (tokenType === TokenTypes.FHIRTYPE) {
            ...
            if (this._isInActionBlock) {
              ...
              this._pendingTokens.push(
                this.createToken(TokenTypes.ACTION_FHIR_TYPE, fhirType, typeStart, typeStartColumn),
              );
            ...
            this._pendingTokens.push(
              this.createToken(TokenTypes.FHIR_VALUE_TYPE, fhirType, typeStart, typeStartColumn),
            );
            return this.createToken(tokenType, text, start, startColumn);
          }
        } else {
